...
3. **Чтение — поллинг по курсору списка диалогов**, mark-read **выключен**:
   ...
   > **Не подтвердилось (11.08.2026).** Не звать `markAsReadMessages` мало:
   > сообщение помечает прочитанным сам `getMessagesHistory`, то есть любой
   > дозабор истории. Собеседник видит «просмотрено», а владелец сессии всё
   > время обхода числится на сайте в сети. Отсюда два следствия в бою:
   > получатель у сайт-аккаунта ровно один (второй бы вычитывал пустоту) и
   > переписку читают только с согласия человека (`sessions.talks_scan`,
   > миграция v10) — молчание согласием не считается.
