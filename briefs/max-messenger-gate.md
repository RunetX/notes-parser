...
1. **Маппинг MAX = «канал + отдельный чат обсуждения»** — верный порт телеграм-модели. У каналов MAX **нет нативных комментариев**, поэтому «автофорвард» (связку пост↔обсуждение) бот делает сам. *(27.07.2026 MAX анонсировал нативные комментарии в каналах, но в Bot API они не транслируются — эксперимент в §5; схема остаётся в силе.)*
...
| **Нет нативных комментариев к каналу** | «Комментарии под постом» реализуют сторонние боты (ze-post и пр.): кнопка «💬 Обсудить» → обсуждение в **отдельном чате**. С 27.07.2026 нативные комментарии есть в клиентах, но **не видны Bot API** (§5) |
...
| 3 | `maxx`: Sink + канал/чат-обсуждения + bridge | да | Заметка → канал+чат; ответ в MAX → комментарий на сайт | **готово** (зеркало проверено живьём; мост — long-poll + bridge.Core, живой прогон ждёт) |
| 4 | Config-гейт + дуал + TG off по умолчанию | нет | Один конфиг гоняет MAX/оба; дефолт — только MAX | **готово** |
| 5 | РюмкинЪ-MAX (ЛС: /login, заметки, подписки) | да | Полный ЛС-функционал в MAX | **готово** (dmbot.Logic через тот же бот; живой прогон ждёт) |
...
**Снято 27.07, эксперимент «нативные комментарии» (в день анонса в канале «MAX Анонсы»):** в клиентах MAX появилась опция включения комментариев к постам канала — без comment-ботов. Проверено на живом демоне (debug-лог неопознанных апдейтов в `maxx.Dispatch`), тестовый канал −77323633892635, бот добавлен админом:
1. `bot_added` по каналу приходит штатно.
2. **Новый факт:** бот-админ видит чужие посты канала как `message_created` с `recipient.chat_type=channel` (mid обычного вида, `link` пуст).
3. **Сами нативные комментарии не порождают никаких апдейтов Bot API** — два тестовых комментария под постом не дали ни нового `update_type`, ни `message_created` из скрытого чата. SDK v2.2.4 про них тоже ничего не знает.

**Вывод:** остаёмся на текущей схеме (копия заметки в чат обсуждения + кнопка «Обсудить») — она к тому же единственная даёт мост «ответ → комментарий на сайт», чего нативные комментарии без API не дадут в принципе. Debug-лог неопознанных апдейтов остаётся в проде как инструмент наблюдения: когда MAX докатит комментарии до Bot API, новые типы всплывут в нём сами (достаточно временно включить `log_level: "debug"` после анонсов).
