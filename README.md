...
| **Личная переписка** | входящие ЛС сайта приходят в мессенджер, ответ реплаем уходит обратно; `/talks`, `/talk`, `/delivery`. Получатель у сайт-аккаунта ровно один |
...
> [!WARNING]
> У сайт-аккаунта ровно один получатель личных сообщений: чтение истории
> помечает сообщение прочитанным на самом сайте, поэтому второй сессии того же
> человека достанется пустота. Кому носить ЛС — решает `/delivery` или вопрос
> кнопками при обнаружении дубля.
