...
- Packages:
  ...
  - `asr` — автораспознавание голосовых в тредах: голосовое (и кружок) в
    группе обсуждения расшифровывается и уходит реплаем на исходное сообщение;
    что делать с текстом, решает автор (ни кнопок, ни состояния).
    **Только Telegram и только группа обсуждения.** Посты канала (приходят в
    группу автофорвардом) пропускаются, и MAX не подключён СОЗНАТЕЛЬНО: в MAX
    расшифровка встроена в клиент (кнопка «Т» и на канале, и в чатах, без
    всякой подписки), в Telegram она только по Premium — там фича и нужна.
    Не «доделывать» MAX-сторону: это оплаченный дубль бесплатного. Побочные
    факты на будущее, если решение пересмотрят: у аудио-вложений MAX нет
    длительности (только url и token), то есть потолок и квота в секундах
    без скачивания файла не считаются, а расшифровка приезжает в поле
    ...
    `asr.Job`), так что подключение второго мессенджера остаётся дешёвым,
    если у него своей расшифровки не окажется. Телеграм-сторона —
    `tgx.VoiceHandler` + `Mirror.DownloadFile`/`ReplyText`
