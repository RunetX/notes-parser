...
- Packages:
  ...
  - `digest` — еженедельный дайджест: слот выпуска (суббота 09:00 Нск, окно
    ...
    (полуручной цикл).
    **Выпуск выходит НА ПЛОЩАДКЕ нативной заметкой** (22.08.2026, D7): у
    сводки про сообщество появляется адрес и тред там, где сообщество живёт, а
    в Telegram и MAX её несёт `platout`, как всякую написанную здесь. Ответ из
    канала попадает в тот же тред (Ш5б) — до этого пост дайджеста обсуждался в
    автофорварде, которого мост не опознаёт, и обсуждение не доходило никуда.
    Плата принята сознательно: сплита серии по 3500 рун больше нет (длинный
    выпуск в канале обрежется со ссылкой на оригинал), и ссылки ведут на
    площадку, а не в треды мессенджеров. Тело — плоский текст со знаками НГС
    (`RenderPlatform` + `chantext.ToSiteMarkup`): ссылки с подписью в заметке
    не существует, поэтому подпись и адрес печатаются рядом. Публикация
    идемпотентна по `message_targets` (приёмник `platform`, kind `digest`),
    свежий выпуск закрепляется наверху ленты и снимает закреп с прошлого
    (`PinIssue`; мест всего `MaxPinned` = 5). Подписант — участник площадки
