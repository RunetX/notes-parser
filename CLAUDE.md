...
- Вход на площадку (Ш4): **код в поле «о себе» анкеты НГС** — человек называет
  номер анкеты, получает `T3H-XXXX-XXXX`, вставляет его в анкету, мы читаем её
  анонимно и сверяем. Запасной путь — приглашение:
...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    как на НГС. Ядро вход не показывает, а только исполняет, поэтому у морды
