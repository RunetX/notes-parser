...
The project is being rewritten in Go (working name `lovegw`, lives in `go/`); the Python version below is the running legacy reference. Rewrite status: M1 (site client + parsers), M2 (SQLite store + legacy importer), M3 (channel mirroring + auto-forward capture), M4 (reply bridge + sessions), and M5 (DM bot РюмкинЪ) are done; subscriptions polish and Docker/deploy (M6–M7) are pending — see the roadmap in the git history / plan discussion.
...
- Packages: `love` (site client/parsers/auth/sessions), `store` (SQLite, the single source of truth — every write is write-through, so state survives kill -9), `tgx` (go-telegram/bot wrapper: per-chat limiters, 429 retry, HTML compose), `mirror` (feed watcher + one goroutine per active note with adaptive poll interval; consumes a `Sink` interface so it's messenger-agnostic — MAX could be a second sink), `bridge` (auto-forward capture + reply→site comment, at-most-once via `processed_replies`), `dmbot` (РюмкинЪ; dialog state persisted in `dialog_states`). Reply→site and note-post reuse imported cookie sessions; a 401/403 marks the session invalid and DMs the user to re-`/login`.
