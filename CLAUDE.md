...
- Веб-морда площадки: `go run ./cmd/lovegw web [-config path]` (пакет `web`,
  ...
  `platform.listen` / `base_url` / `media_dir` / `platform.preview_key`.
  **Чтение открыто всем** (18.08.2026), вход — кнопкой в правом верхнем углу
  шапки; пустой ключ означает «войти пока некуда», а не «закрыто». От
  поисковиков страницы закрыты по-прежнему. При расхождении версии схемы
...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    как на НГС. До настоящего входа (Ш4) там общий ключ: в куке лежит его хеш,
    пустой ключ означает «войти некуда» и чтения не закрывает, а даёт вход
    ровно одно — метку «свой» в шапке. Индексация запрещена и заголовком, и
    ...
    переключатель это форма с POST, а не ссылка (как и выход). **Чего из
...
- Config `go/config.json` (gitignored, template `go/config.example.json`);
  ...
  `preview_key` — общий ключ ВХОДА до Ш4, тоже секрет и тоже в env; чтения он
  не закрывает, пустой означает «войти пока некуда»).
  The `messengers`
