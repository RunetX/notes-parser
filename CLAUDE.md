...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    (записанная страница / стили / экран владельца). Курсорное пролистывание
    ...
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    год); медиа в бою отдаёт Caddy мимо Go. **Ворота** (`gate.go`) — общий ключ
    до настоящего входа (Ш4): в куке лежит хеш ключа, пустой ключ закрывает
    всё. Темы — токены `:root` плюс `data-theme` из куки; переключатель это
    форма с POST, а не ссылка. **Чего из оригинала нет намеренно**: лайки,
    жалобы, огонёк «онлайн», боковая колонка новостей НГС — первые три площадка
    не заводит по решению эпика E, последняя чужая. **Чего нет пока**: цвет
    ника по полу (`#ef4e4f` женский, `#448dc8` мужской) — пола площадка не
    хранит, классы `_female`/`_male` в разметке заведены заранее.
