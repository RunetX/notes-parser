...
- Добор восстановленного (`platform import-restored -archive archive.db`, тот же
  ...
  «Димон» и «Димон_Таибычев». Ребро находится у 6 945 реплик из 11 577,
  обращение при этом срезается в ребро; не разрешилось — остаётся текстом.
  Заодно возвращается подпись 181 заметке 2010 года, которую раскатка записала
  анонимной (автора без анкеты она приравнивает к анониму), и тредам ставится
...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    не красит. Своего синтаксиса площадка по-прежнему не заводит — написанное
    сегодня показывается как есть. **Смайлы сайта** (`:::popcorn:::`,
    ...
    **CSP без `unsafe-inline` запрещает и атрибут `style`**, поэтому глубина ветки приезжает КЛАССОМ `d1…d12`, а
    ...
    переписывать их ради вида нечестно. Статика вшита в
    ...
    оригинала нет намеренно**: жалобы, огонёк «онлайн», боковая колонка новостей
    НГС — первые две площадка не заводит по решению эпика E, последняя чужая.
    ...
    было бы пять тысяч кнопок на странице.
