...
- Packages:
  ...
  - `asr` — автораспознавание голосовых в тредах: голосовое (и кружок) в
    ...
    что делать с текстом, решает автор (ни кнопок, ни состояния). Посты канала
    (приходят в группу автофорвардом) сознательно пропускаются: и Telegram, и
    MAX показывают там собственную расшифровку по кнопке. Пакет
