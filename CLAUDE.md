...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    В каналы мессенджеров нативный текст уходит ПЛОСКИМ (`platout` отдаёт
    приёмнику строку, а композер её экранирует) — то есть `[b]` там виден
    буквально; чинится это только на стороне приёмников.
    ...
    **Справка и правила** (`/help`, `help.go`) — единственная страница, которую
    ...
    мёртвыми. **Адреса в чужом тексте не кликабельны** (`linksClickable`,
    18.08.2026): инструмента контроля пока нет, а в базе 10,7 млн чужих реплик
    за тринадцать лет — текст адреса остаётся целым, ссылкой он не становится;
    в текстах согласий ссылки живые, это наш документ.
