...
- Веб-морда площадки: `go run ./cmd/lovegw web [-config path]` (пакет `web`,
  ...
  `platform.listen` / `base_url` / `media_dir` и **обязательно**
  `platform.preview_key`: пустой ключ означает «не пускать никого», а не
  «пускать всех» — страницы отвечают 403, живут только `/healthz` и
  `/robots.txt`. При расхождении версии схемы стартовать отказывается.
...
- Packages:
  ...
  - `web` — SSR-морда площадки (лента, страница заметки, дерево и плоский вид,
    темы). Ходит в `platform` напрямую, а не в свой же API по петле — но через
    **интерфейс `Store`**, и это же список того, что морда умеет делать с
    ...
    отступ считает `calc()`; из того же запрета следует «ни npm, ни CDN».
    Обращение «Ник, » дорисовывается ПОКАЗОМ из ребра `reply_to_id` (в теле его
    нет) — иначе переименование и обезличивание не меняли бы подпись в чужих
    ответах. Пролистывание везде по ключу, не по OFFSET: лента живая, а тред
    режется на страницы по 100 (треды на 848 реплик роняют сам НГС). Статика
    вшита в бинарник и адресуется хешем содержимого (`style.<hash>.css`,
    `immutable` на год); медиа в бою отдаёт Caddy мимо Go, обработчик в
    `assets.go` — для разработки. **Ворота** (`gate.go`) — общий ключ до
    настоящего входа (Ш4): в куке лежит хеш ключа, пустой ключ закрывает всё.
    Темы — токены `:root` плюс `data-theme` из куки; переключатель это форма с
    POST, а не ссылка (GET на смену состояния сам себя нажимает в префетче).
...
- Config `go/config.json` (gitignored, template `go/config.example.json`);
  ...
  `LOVEGW_PLATFORM_PREVIEW_KEY` (секция `platform`;
  ...
  `preview_key` — общий ключ доступа к страницам до Ш4, тоже секрет и тоже в
  env; пустой закрывает площадку целиком).
  The `messengers`
