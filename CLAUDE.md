...
- Packages:
  ...
  - `platout` — ОБРАТНОЕ направление: написанное на площадке уходит в каналы
    ...
    **Учёт отправленного — в SQLite `message_targets`**, а не в Postgres: там
    ...
    (`kbd.Subscribable`): они живут в SQLite и знают только заметки НГС. Чего
    пакет НЕ делает — обратного пути: ответ, написанный в мессенджере,
    по-прежнему идёт мостом на НГС, а не на площадку.
