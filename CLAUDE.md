...
- Packages:
  ...
  - `platmod` — автомат модерации: он снимает ОБЪЁМ, решения о людях остаются
    ...
    поштучно платили бы за правила каждый раз) дешёвой моделью
    `claude-haiku-4-5`; **effort ей передавать нельзя — она отвечает на него
    ошибкой запроса**, отсюда и отдельная секция конфига вместо общей
    `llm.model`. Контекста разговора модель не получает вовсе: закрытый список
    ...
    обязательна независимо от классификатора: нет ключа Claude — очередь просто
...
- Config `go/config.json` (gitignored, template `go/config.example.json`);
  ...
  `LOVEGW_MODERATION_DAILY_REQUESTS` (секция `platform`;
  ...
  каждую публикацию, а очередь модератора живёт и без него; `model` —
  `claude-haiku-4-5`, и `effort` при ней обязан быть ПУСТ (Haiku 4.5 отвечает на
  него ошибкой запроса); `daily_requests` считает ЗАПРОСЫ, а не публикации, —
  это прямая единица счёта денег. Инструменты модератора от секции не зависят
  вовсе: они часть ядра и морды. Подсекция `bus` — шина событий (эпик F):
