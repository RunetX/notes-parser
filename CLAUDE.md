...
- **Storage is SQLite** (`modernc.org/sqlite`, CGo-free) — the single source of
  ...
  живут в env и в шифрованных колонках `sessions`/`accounts`).
...
- Packages:
  ...
  - `pulpit` — «амвон»: собственная реплика владельца под каждой новой заметкой
    ...
    Формы просят не ЗАМЕТИТЬ смешное, а ВЛЕЗТЬ в ситуацию (влезаю в ситуацию,
    довести до конца, не тот эксперт, инструкция, сценка, я тоже дурак,
    внезапная конкретика, встречный вопрос) — первый набор просил заметить, и
    модель выдавала наблюдение сверху: остроумно и не смешно. Оттуда же запрет
    афоризма («дело не в X, а в Y», «тревожит не то, что…») — он в валидаторе,
    а не только в промпте. **Реплика пишется в два прохода**: черновик, затем
    правка шутки (`punchUp`, свой системный промпт — убрать пояснение после
    удара, заменить общее конкретным, добавить добивку); провал правки не
    отменяет реплику, уходит черновик. Первым голосом был проповедник, и он прожил ровно
    ...
    (сбой загрузки и снесённая заметка промахом НЕ считаются) или две
    вычищенные реплики гасят фичу, диагноз подтверждается `love.ProfileControl`,
    админу уходит ЛС. **Автовосстановления нет**: срок запрета неизвестен,
