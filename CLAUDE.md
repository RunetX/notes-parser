...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **CSP без `unsafe-inline` запрещает и атрибут `style`**, поэтому глубина ветки приезжает КЛАССОМ `d1…d12`, а
    ...
    привычное…», один человек старым ником и новым (найдено владельцем
    18.08.2026). Правило показа поэтому такое: **автор назвал адресата сам —
    второго обращения не дорисовываем**, а выделяем жирным написанное им
    (`address.go`); ссылки у него нет, потому что ник исторический, а ребро может
    указывать на другого. Замер по боевой базе (289 776 реплик с ребром): 60 %
    начинаются с «X, », и лишь 2 % из них — зачины вроде «Кстати». Размен назван
    честно: ребро точнее текста (92 % против 48 %), но показ не вправе прятать
    слова автора, а «оба сразу» — это и есть та поломка, с которой всё началось. Статика вшита в
