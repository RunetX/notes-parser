...
- Packages:
  ...
  - `rullm` — тот же контракт `GenerateJSON`, но поверх **Yandex AI Studio**
    ...
    Дефолт `yandexgpt-lite/latest` — замер на смешанной пачке дал те же
    вердикты, что `gpt-oss-120b`, вдвое дешевле по токенам.
...
- Config `go/config.json` (gitignored, template `go/config.example.json`);
  ...
  (`yandexgpt-lite/latest` либо `claude-haiku-4-5`), и `effort` обязан быть ПУСТ
