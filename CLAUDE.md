...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **Шапка** — название площадки (`SiteName`, «Зазеркалье»), вкладки «Лента» и
    ...
    крайних — `overflow: hidden` там нельзя, под меню участника выпадает список. Справка стоит и в шапке, и в
    подвале: правила нужны переехавшему в первый же заход, а искать их внизу он
    не догадается.
    ...
    **CSP без `unsafe-inline` запрещает и атрибут `style`**, поэтому глубина ветки приезжает КЛАССОМ `d1…d12`, а
    ...
    не заводит, ник, фото и согласия живут на той же `/me`. **Фото — кнопкой
    ...
    **Живой добор** (`fresh.go`, 23.08.2026) — вторая половина: получив сигнал,
    ...
    переключатель это форма с POST, а не ссылка (как и выход). Тем **четыре, и
