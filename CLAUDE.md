...
- Packages:
  ...
  - `web` — SSR-морда площадки. Мера здесь не «красиво», а **«человек не
    ...
    **Живой добор** (`fresh.go`, 23.08.2026) — вторая половина: получив сигнал,
    ...
    анонима. Скрипт только выбирает МЕСТО: в дереве — после всей ветки
    адресата (`data-parent`/`data-depth`), в линейном виде и в ленте — сверху,
    ...
    (`data-fresh` → `?after=` → `X-Fresh-After`): у треда это id, у ленты пара
    «время, id» — по одному id граница разъехалась бы на полосах
    идентификаторов. Запрос свой (`CommentsSince`/`NotesSince`), а не «дай тред
