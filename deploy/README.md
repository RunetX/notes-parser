...
> **С 17.08.2026 всё живёт на одном хосте вместе с площадкой.** Демон переехал
> на VPS площадки (`t3h.ru`), и compose там **один на семь сервисов** —
> [platform/docker-compose.yml](platform/docker-compose.yml). Причина не в
> удобстве: площадку наполняет fan-out ВНУТРИ демона, значит писать в её
> Postgres обязан тот же процесс, что зеркалит, а портов у базы наружу нет.
> Прежний `deploy/docker-compose.yml` удалён — он описывал раскладку, которой
> больше нет (в истории git он на месте).
>
> Что от этого поменялось в процедурах: **образ на хосте больше не собирается**.
> Бинарник собирается на рабочей машине и приезжает готовым
> ([platform/Dockerfile.prebuilt](platform/Dockerfile.prebuilt)) — на одном ядре
> `go build` идёт 25 минут и уронил бы всё, что на этом хосте работает. Актуальная
> процедура выкатки, миграции схемы и сверка — в
> [platform/README.md](platform/README.md). Ниже — общий рунбук: требования к
> хосту, конфиг, шифрование сессий, диагностика; шаги про сборку из исходников
> на хосте относятся к прежней раскладке.
...
- `modwatch` — наблюдатель за действиями модерации (только чтение сайта, своя
  БД `data/modwatch.db`, см. шаг 10);
- `guests` — визиты в анкету владельца (ходит под его сессией из `lovegw.db`,
  пишет в ту же `modwatch.db`);
- `activity` — присутствие людей на сайте, детектор запретов в «Заметки»
  (анонимный обход анкет; круг берёт из `lovegw.db` только чтением реплик).

Для работы зеркала нужен лишь первый — остальные три можно не поднимать.
...
`user@ХОСТ` здесь и ниже — плейсхолдер, и это **решение, а не пробел**:
репозиторий публичный, поэтому адрес боевого хоста и путь к ssh-ключу в нём не
хранятся. Они у оператора (и в локальной памяти проекта). Не найдя их в
репозитории, не считайте, что выкатка невозможна — спросите.
...
1. **Бэкап БД** перед первым стартом с talks: `cp data/lovegw.db data/lovegw.db.bak`.
   ...
   безопасен).
2. Прогнать чек — покажет, чью переписку обход читает:
   ```sh
   docker compose run --rm lovegw doctor -config /config.json
   # talks/telegram: мультисессия: N валидных сессий (обходим согласившихся)
   # talks/чтение: читаем переписку K из N сайт-аккаунтов (ждут ответа …)
   ```
3. Применить конфиг и перезапустить: `docker compose up -d --build`.
4. Переписку читают **только с согласия**: в первом цикле бот один раз спросит
   каждого кнопками, и до ответа ЛС не приходят никому — это ожидаемое
   состояние, а не поломка. Согласившемуся первый цикл разошлёт его
   **непрочитанные** входящие (прочитанное на сайте не трогается), дальше —
   только новое. Двусторонний режим активен сразу.
...
**суббота 09:00 Нск**: он задаёт не только время публикации, но и шов недели,
а в утренней тишине срез не рассекает живой разговор — см. комментарий
`digest.DefaultTZ`). Демон в слот выпуска строит черновик и материалы в
...
суммарный темп запросов к love.ngs.ru складывается из всех работающих сервисов.

Рядом с ним в ту же `modwatch.db` пишут ещё два сервиса:

```sh
docker compose up -d guests activity
docker compose run --rm activity modwatch -config /config.json -db /data/modwatch.db activity report
docker compose run --rm activity modwatch -db /data/modwatch.db activity log -near "2026-08-12 19:38"
```

- `guests` — визиты в анкету владельца. Ходит **под его сессией** из
  `lovegw.db`, поэтому при включённом шифровании ему нужен ключ (см. «Секреты»).
  Снимать надо регулярно: сайт держит одну строку на человека и при повторном
  визите затирает прежнюю.
- `activity` — присутствие людей на сайте. Обход анонимный (ключ не нужен, в
  чужих гостях следа не оставляет), круг — активные комментаторы из зеркала.
  `activity report` показывает, кто замолчал: «ходит молча» — запрет в
  «Заметки», «не заходит» — ушёл сам. Первые сутки после запуска отчёт будет
  говорить «мало данных» и это правильно: одна свежая отметка не отличает
  закрытого от вернувшегося из отъезда, нужен след на нескольких сутках.

## Шаг 11 — амвон: своя реплика под каждой новой заметкой (опционально)

Служба `pulpit` внутри демона (отдельного сервиса в compose нет) пишет от имени
владельца короткую шутку под каждой новой заметкой сайта — и старается успеть
первой: первый чужой комментарий приходит через медианные 164 секунды, а весь
наш путь (обнаружение, генерация, отправка) укладывается в 25–45.

**Голос менялся.** Первым был проповедник — его единственную реплику сняли
через 15 минут, а владельцу выдали сутки запрета писать. Нынешний голос
(прикольщик) шутит над положением, а не над человеком, и молчит под настоящей
бедой. Прежде чем включать после паузы, стоит прогнать `pulpit draft` по
вчерашним заметкам: на сайт он ничего не отправляет.

Включение: секция `pulpit` в `config.json` — `"enabled": true` и
`owner_profile_id` (номер анкеты владельца, `/profile/<id>/`). Ключ Claude тот
же, что у дайджеста (`LOVEGW_LLM_KEY`), запросы идут через `LOVEGW_TG_PROXY`.
Владелец должен быть **залогинен ботом** (`/login`): реплика уходит под его
сессией, найденной по `owner_profile_id`.

```sh
docker compose run --rm lovegw pulpit -config /config.json draft 312900   # черновик, на сайт ничего не уходит
docker compose run --rm lovegw pulpit -config /config.json status         # включён ли, что писал последним
```

**Выключить без ssh** — в ЛС боту команд: `/pulpit` → «⛔ Выключить». Тумблер
живёт в БД (`settings['pulpit.enabled']`), а не в конфиге, поэтому переживает
рестарт и не требует правки смонтированного `/config.json`. Включение — та же
команда; после срабатывания предохранителя оно идёт через подтверждение.

**Выключить совсем** — `"enabled": false` в секции `pulpit` (или
`LOVEGW_PULPIT_ENABLED=false` в `secrets.env`) и рестарт: службы не будет вовсе.

Предохранитель гасит фичу сам, если три реплики подряд не появились в тредах
(так выглядит запрет писать в раздел: анкета жива, страницы читаются, сайт
отвечает 200), если две подтверждённые реплики вычистили или если сессия/анкета
оказались закрыты. Об этом приходит ЛС админу. **Автовосстановления нет
намеренно**: срок запрета неизвестен, и вторая попытка под запретом стоит
второго бана — включать обратно только руками.
...
Ключ нужен сервисам `lovegw` и `guests` (оба читают сессии); `modwatch` и
`activity` ходят на сайт анонимно, им ключ не нужен — `activity` хоть и
открывает `lovegw.db`, но читает оттуда только реплики, мимо `sessions` и мимо
проверки `openStore`. **Хранить ключ отдельно от бэкапов базы** —
