...
На боевом VPS (1 vCPU, 956 МБ RAM + 2 ГБ swap) полная сборка с нуля — около
15 минут; тянет её компиляция дерева зависимостей, тяжелее всех
`anthropic-sdk-go`. Два места, где время терялось, закрыты и трогать их не надо:
...
docker buildx du               # сколько кэш занимает (если есть buildx)
```
