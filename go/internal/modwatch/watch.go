...
// Лента опрашивается вровень с зеркалом (`feed_interval_s: 60`): 12.08.2026
// заметка «Шуруповёрт» прожила меньше такта в 90 с — зеркало успело её
// запостить, наблюдатель не увидел вовсе. Заметку короче такта не поймает
// никакой период, но сравнять сетку с зеркалом стоит одного запроса в минуту.
const (
	DefaultFeedInterval   = 60 * time.Second
