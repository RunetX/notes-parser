...
// branchLimit — сколько строк разговора показывать модели. Выведен из боевого
// числа, а не повторяет его: расходиться нельзя — иначе калибровка мерила бы
// голос при одном контексте, а боевая генерация шла бы при другом.
//
// Прежде здесь стояла своя двадцатка с этим же доводом в комментарии, и довод
// её не уберёг: 01.09.2026 выяснилось, что миры разошлись не числом, а СОСТАВОМ
// окна (narod.windowOfThread) — калибровка брала цепочку предков, бой не брал
// вовсе. Число теперь одно на оба мира физически, а состав стережёт тест.
const branchLimit = narod.ThreadWindow
