...
func TestValidate(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Живой черновик 16.08.2026: правка отрезала фразу и оставила от неё
			// слово отдельной строкой. В 108 знаков реплики оно входило.
			name: "обрывок после панча",
			text: "Выгребная яма, свой дом, огород - вот это и есть с одной грядки.\nПапа не уточнил, с какой.\nгрядки.",
			form: "буквально", cfg: cfg, bad: true,
		},
		{
			// Живой черновик 16.08.2026: хвост отвалился от модели уже после
			// точки, поэтому проверка на смешанное слово его не видела.
			name: "латинский огрызок после точки",
			text: "Вселенная четыре месяца слала сигналы. Календарь справился бы быстрее.pt",
			form: "буквально", cfg: cfg, bad: true,
		},
		{
			// Настоящее слово латиницей длиннее огрызка и брака не образует.
			name: "живое слово латиницей",
			text: "Он тебе и в WhatsApp то же самое напишет, только с опечатками.",
			form: "буквально", cfg: cfg,
		},
		{
			// У добивки есть своё слово — она не повтор уже сказанного.
			name: "короткая добивка годится",
			text: "Три года молчал, а тут родинка. Отличный повод.\nВрач ждет.",
			form: "буквально", cfg: cfg,
		},
		{
