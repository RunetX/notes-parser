...
type Service struct {
	...
	// ленты только помечаются, иначе рестарт демона выдал бы пять реплик разом
	// подряд под старьё.
	cold bool
	...
	replyAt     time.Time
	replyCursor int
	// sendFails / lastSendFail — не дошедшие подряд POST'ы и время последнего.
	// Пока сайт не принимает запись, новые заметки не берём: каждая попытка
	// стоит генерации и правки в Claude, а в тред не попадает ничего (см.
	// writePaused). Состояние в памяти, а не в БД, намеренно: это экономия, а
	// не предохранитель, и рестарт вправе стоить одной пробной попытки.
	sendFails    int
...
func (s *Service) Run(ctx context.Context) error {
	...
	for {
		...
		case n := <-s.events:
			...
			// без этого под ними разом появилась бы очередь реплик.
...
func (s *Service) cycle(ctx context.Context) {
	...
	if enabled && !s.wasEnabled {
		...
		// старыми заметками ленты разом появится очередь реплик.
		s.cold = true
	}
