...
// replyMaxRunes — потолок ответа: он короче проповеди по замыслу.
const replyMaxRunes = 220

// Темп обхода веток. Ответы никуда не спешат (в отличие от самой проповеди,
...
func (s *Service) sendReply(ctx context.Context, row store.PulpitComment, c love.Comment, nick string) bool {
	...
	if err := s.site.PostComment(ctx, cookies, row.NoteID,
		...
		// Как и с проповедью: не откатываем и не повторяем — сайт мог принять.
