...
type Platform struct {
	pool *pgxpool.Pool
	// media — хранилище файлов, если оно поднято. Ссылка обратная (хранилище и
	// так держит платформу) и нужна ровно одному: УБОРКЕ. Снятая фотография
	// профиля обязана унести за собой байты — это единственное место проекта,
	// где хранилище чистится, — а сносить файл должен тот же код, что его
	// кладёт. Ставит её NewMediaStore, поэтому лишнего вызова при сборке не
	// появляется; у разовой команды без хранилища она пуста, и уборка тогда
	// оставляет сироту — ссылок на неё нет, вреда тоже.
	media *MediaStore
}
