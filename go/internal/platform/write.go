...
func writeGuard(ctx context.Context, q querier, userID int64) error {
	...
	switch {
	// Служебная анкета площадки (KindService) писать ВПРАВЕ: под ней выходит
	// недельный выпуск, и автор у него не человек, а сам оператор. Второй kind
	// в этой строке — единственное послабление, которое ей понадобилось: всё
	// остальное участниковое хозяйство её и так не касается, потому что
	// спрашивается по `kind = KindMember` и потому отвечает «нет» само —
	// колокольчик (fanOut), вынос на НГС (enqueueNGS), права вошедшего в морде.
	// Ровно этим служебная анкета отличается от ЖИТЕЛЯ: житель обязан быть
	// неотличим от участника везде, кроме согласий и песочницы, — поэтому он
	// участник с признаком; площадку же, наоборот, надо исключить отовсюду, где
	// речь про человека, — поэтому она отдельный вид.
	case anonymized != nil, kind != KindMember && kind != KindService:
		return ErrNotMember
...
// notASubject — этому автору согласий не подписывать, потому что субъекта
// персональных данных за ним нет вовсе. Отдельным запросом, а не полем в
// writeGuard: спрашивают про это ровно в одном месте, и оно про согласия, а не
// про право писать вообще.
//
// Таких двое, и поводы у них РАЗНЫЕ, хотя ответ один.
//
// ЖИТЕЛЬ (users.persona, эпик «народ») — у персонажа персональных данных нет,
// и подпись от его имени была бы записью о согласии, которого никто не давал.
//
// СЛУЖЕБНАЯ АНКЕТА ПЛОЩАДКИ (KindService) — это сам ОПЕРАТОР. Согласие субъект
// даёт ОПЕРАТОРУ; подпись оператора самому себе — не документ, а выдумка, и
// хранить её в доказательственной таблице нельзя ровно по тому же доводу.
// Право публиковать даёт ей не подпись, а то, что она заведена рукой
// администратора (`platform migrate`): за выпуск отвечает оператор.
func notASubject(ctx context.Context, q querier, userID int64) (bool, error) {
	var (
		persona bool
		kind    Kind
	)
	if err := q.QueryRow(ctx,
		`SELECT persona, kind FROM users WHERE id = $1`, userID).Scan(&persona, &kind); err != nil {
		return false, fmt.Errorf("вид автора %d: %w", userID, err)
	}
	return persona || kind == KindService, nil
}
...
func consentGuard(ctx context.Context, q querier, userID int64) error {
	// СОГЛАСИЕ ДАЁТ СУБЪЕКТ, поэтому у автора, за которым субъекта нет вовсе,
	// подписи не спрашивают: житель эпика «народ» и служебная анкета самой
	// площадки — оба повода расписаны у notASubject.
	//
	// Исключение стоит здесь, а не в вызывающих: путей публикации несколько
	// (форма, мост из мессенджера, служба народа, выпуск дайджеста), и второй
	// список правил рядом с этим однажды разошёлся бы — тот же довод, по
	// которому сюда переехала и сама проверка редакции.
	exempt, err := notASubject(ctx, q, userID)
	if err != nil {
		return err
	}
	if exempt {
