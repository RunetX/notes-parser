...
const (
	...
	// SubjectProfile — карточка человека: рассказ о себе, город, занятие и его
	// фотографии (эпик M). ОДНА строка на человека, а не по строке на поле и не
	// по строке на снимок, и доводов три. Очередь растёт числом людей, а не
	// числом их правок, — цель владельца «единицы строк в сутки». Модератор
	// судит то, что ВИДИТ, а видит он страницу целиком. И обезличивание уже
	// переносит author_id на могилу — при subject_id = users.id карточка
	// указывает на опустошённую строку, а не на имя.
