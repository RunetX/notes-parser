...
func enqueueNGS(ctx context.Context, q querier, kind string, objectID, authorID, noteID int64, stage bool) error {
	...
	// `kind = $2` (KindMember) отсекает заодно и СЛУЖЕБНУЮ АНКЕТУ ПЛОЩАДКИ, под
	// которой выходит недельный выпуск, — и это не побочный эффект, а нужный
	// ответ: у площадки нет ни анкеты на НГС, ни сессии там, значит уносить её
	// слова некуда и незачем. Отдельного условия про дайджест поэтому нет
	// нигде: 05.09.2026 выпуск уехал на сайт под именем владельца именно
	// потому, что подписывал его человек, а не она.
	var on bool
	err := q.QueryRow(ctx, `
