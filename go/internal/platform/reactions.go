...
// ReactionCodes — что предлагается нажать, и набор здесь НЕ на вкус.
//
// Первая версия была шестёркой, собранной по смыслу, и владелец справедливо
// возразил: частые знаки должны быть частыми, а не выбранными. Замер 18.08.2026
// по БОЕВОМУ корпусу (выборка 5 % от 10,77 млн комментариев, коды считаны
// регуляркой) дал порядок, который тут и стоит:
//
//	crazy2 7647, agree 3008, popcorn 2525, flowers 1265, shuffle 1189,
//	boogi 940, sad2 769, wow 756, str 587, live 506, respect 402, bottle 372
//
// Дальше частота падает вдвое (tease 340, eek 336), и там же кончается ряд,
// влезающий в строку на телефоне. Смысл кнопок мы не назначаем: эти двенадцать
// знаков люди выбирали сами тринадцать лет, и «str» с «live» в списке именно
// поэтому — нам их значение неочевидно, а сообществу очевидно.
var ReactionCodes = []string{
