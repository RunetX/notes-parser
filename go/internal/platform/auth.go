...
// Вход.
//
// Основной путь — код в поле «о себе» анкеты НГС: человек называет номер
// анкеты, мы показываем код, он вставляет его в анкету, мы читаем её АНОНИМНО и
// сверяем. Ни пароля, ни бота, ни сотрудничества НГС не нужно, и это работает у
// забаненного в «Заметках»: запрет писать ничего не убирает с площадки, анкета
// жива и редактируется.
//
...
// запросил её владелец. Второй половиной служит кука, выданная в момент выдачи
// кода: она подтверждает «этот челлендж начал я». Нужны обе — иначе посторонний,
// заметивший чужой код, входил бы под чужим именем одним нажатием.
//
...
func (p *Platform) StartProfileChallenge(ctx context.Context, profileID int64) (Challenge, error) {
	...
	if _, err := tx.Exec(ctx,
		...
		IdentityNGS, subjectOf(profileID)); err != nil {
		return Challenge{}, fmt.Errorf("выдача кода анкете %d: %w", profileID, err)
	}
	if _, err := tx.Exec(ctx, `
		INSERT INTO auth_challenges (kind, subject, code_sha, expires_at)
		VALUES ($1, $2, $3, $4)`,
		IdentityNGS, subjectOf(profileID), sum, expires); err != nil {
		return Challenge{}, fmt.Errorf("выдача кода анкете %d: %w", profileID, err)
	}
...
func (p *Platform) VerifyProfileChallenge(ctx context.Context, profileID int64, code, aboutMe string) error {
	...
	err = tx.QueryRow(ctx, `
		SELECT id, code_sha, attempts FROM auth_challenges
		 ...
		 FOR UPDATE`, IdentityNGS, subjectOf(profileID)).Scan(&id, &stored, &attempts)
...
func (p *Platform) ProfileVerified(ctx context.Context, profileID int64, code string) (bool, error) {
	...
	err := p.pool.QueryRow(ctx, `
		SELECT code_sha FROM auth_challenges
		 ...
		 ORDER BY verified_at DESC LIMIT 1`, IdentityNGS, subjectOf(profileID)).Scan(&stored)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, nil
	}
	if err != nil {
...
func (p *Platform) CompleteNGSLogin(ctx context.Context, prof MirroredAuthor, gender Gender) (int64, error) {
	...
	default:
		...
		if _, err := tx.Exec(ctx, `
			UPDATE users
			   SET kind = $2, nick = $3, ngs_avatar_url = $4,
