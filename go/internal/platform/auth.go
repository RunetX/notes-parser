...
var (
	...
	// ErrTooManyAttempts — проверки идут слишком ЧАСТО (challengeMinInterval).
	// Не «попыток больше нельзя»: потолка на число проверок нет с 10.09.2026, и
	// ответ человеку обязан звать подождать минуту, а не начинать заново.
	ErrTooManyAttempts = errors.New("проверки слишком часто")
...
const (
	// ChallengeTTL — сколько живёт код. НЕДЕЛЯ, и это число оплачено замером.
	//
	// Стоял ЧАС, с доводом «человеку нужно открыть НГС, отредактировать анкету
	// и вернуться». Довод верен для правки, которая появляется СРАЗУ, — а
	// правку поля «о себе» сначала одобряет модератор НГС, и одобряет не сразу
	// и не наверняка. Оба факта прожили рядом три недели, причём соседними
	// фразами на одном экране: «Код живёт час» и «правку сначала одобряет
	// модератор». Замер 10.09.2026: с 19.08 вход по коду начали ДЕСЯТЬ человек,
	// дошёл НИ ОДИН, а трое жали «Проверить» по 8, 16 и 20 раз. Пока жил канал
	// лички, этот путь был запасным и им не пользовались; 01.09 личку убрали, и
	// он стал единственным — непроходимым.
	//
	// Неделя, а не сутки: модерация на той стороне — живой человек с
	// выходными, и «начал в пятницу» не должно значить «начинай заново в
	// понедельник». Цена названа честно: код всё это время висит в публичном
	// «о себе». Она невелика — кода в анкете МАЛО, вторая половина
	// доказательства это кука того, кто проверку начал (см. шапку файла), — но
	// экран поэтому и просит убрать строку после входа.
	ChallengeTTL = 7 * 24 * time.Hour
	// challengeMinInterval — не чаще одной проверки в минуту.
	//
	// Пришло на смену абсолютному потолку в двадцать проверок, и разница тут
	// принципиальная. Потолок заводили не против подбора (40 бит по сети не
	// подбирают), а против долбёжа по НГС: каждая проверка — наш запрос к
	// сайту. Но человек, ждущий чужой модерации, жмёт много раз ПО САМОЙ
	// ПРИРОДЕ ОЖИДАНИЯ, — и потолок защёлкивался именно на нём: 10.09.2026
	// участник упёрся в двадцатую проверку и написал в тред, что войти не смог.
	//
	// Темп бережёт тот же запрос к сайту и никого не запирает. Абсолютного
	// потолка рядом нет НАМЕРЕННО: за неделю ожидания любой такой потолок — это
...
func (p *Platform) VerifyProfileChallenge(ctx context.Context, profileID int64, code, aboutMe string) error {
	...
	err = tx.QueryRow(ctx, `
		SELECT id, code_sha, attempts, last_attempt_at FROM auth_challenges
		 WHERE kind = $1 AND subject = $2 AND verified_at IS NULL AND expires_at > now()
		 FOR UPDATE`, ChallengeProfile, subjectOf(profileID)).Scan(&id, &stored, &attempts, &lastAttempt)
	...
	// Слишком частая проверка не засчитывается ВОВСЕ — ни счётчиком, ни
	// отметкой времени: иначе человек, кликнувший дважды подряд, отодвигал бы
	// себе следующую разрешённую проверку каждым лишним нажатием.
	if lastAttempt != nil && time.Since(*lastAttempt) < challengeMinInterval {
		return ErrTooManyAttempts
	}
	if _, err := tx.Exec(ctx,
		`UPDATE auth_challenges SET attempts = attempts + 1, last_attempt_at = now() WHERE id = $1`, id); err != nil {
		return fmt.Errorf("проверка кода анкеты %d: %w", profileID, err)
	}
