...
// которые ничего не просили. Поэтому уходит не текст, а ЛИЧНОСТЬ: строки
// человека переезжают на свежую нативную анкету-«могилу» без имени, и подпись
// под ними меняется везде, включая обращения «Ник, » в чужих ответах. Могила
// своя на каждого — иначе разговоры разных людей слились бы в один голос, и
// тред стал бы враньём; при этом определить, чья она, без нашей помощи нельзя,
// а помощи не будет: соответствие «кто → какая могила» нигде не записывается,
// включая журнал.
//
// Объёма у обезличивания ДВА, и делит их не вкус, а цена прохода.
//
//   - ЗАМЕТКИ человека — это тысячи строк у самого плодовитого автора, они
//     идут по полному индексу notes_author_all и укладываются в срок
//     веб-запроса целиком. Поэтому обезличивание заметок делает кнопка:
//     anonymizeOwnNotes, и зовёт её отзыв согласия (consent.go).
//   - ВСЁ вместе с комментариями — это до 138 тыс. строк и десятки секунд
//     (замер 18.08.2026), то есть в срок веб-запроса не влезает вовсе, а
//     необратимость требует не галочки, а переписки. Поэтому AnonymizeUser
//     остаётся командой администратора. Закон это допускает прямо: на
//     требование субъекта у оператора тридцать дней, а не секунда.
//
// Выгрузка — командой по той же причине, что и полное обезличивание.
...
// newGrave заводит могилу — обычную нативную строку users, помеченную
// обезличенной сразу: так её не тронут ни зеркало (ensureShadow обходит
// обезличенных), ни вход, ни фоновые обходы.
//
// Время у неё огрублено до суток НАРОЧНО. Точное now() совпало бы с
// consents.revoked_at той же транзакции до микросекунды, и соответствие «кто →
// какая могила» — то самое, которого мы не пишем ни в журнал, ни в колонку, —
// оказалось бы записано метками времени.
func newGrave(ctx context.Context, q querier) (int64, error) {
	var grave int64
	err := q.QueryRow(ctx, `
		...
		RETURNING id`, GraveNick, KindShadow).Scan(&grave)
	return grave, wrapf(err, "заведение могилы")
}
...
// anonymizeOwnNotes переносит ЗАМЕТКИ человека на свежую могилу, не трогая ни
// их видимость, ни его комментарии, ни саму учётную запись. Возвращает число
// перенесённых.
//
// Так исполняется отзыв согласия на распространение (consent.go). До 25.08.2026
// отзыв ПРЯТАЛ всё написанное — и скрытая заметка уносила со страниц весь тред,
// то есть чужие ответы на чужие слова. Ровно этим доводом площадка запретила
// себе удаление в модерации («скрываем, а не удаляем») и стирание текстов в
// обезличивании; отзыв согласия был из общего правила единственным исключением.
//
// Комментарии остаются подписанными, и обезличивание от этого СЛАБОЕ: человек
// отвечает в собственной теме под своим ником, и первая же его реплика говорит
// читателю, чья это заметка. Решение владельца 25.08.2026, и цена названа в
// самом тексте согласия (consents/distribution.v3.txt): документ, обещающий
// больше, чем площадка делает, хуже отсутствующего. Полное обезличивание,
// вместе с комментариями, остаётся у AnonymizeUser — по обращению.
//
// Могила заводится СВЕЖАЯ на каждый отзыв: помнить «его прошлую могилу» значило
// бы хранить то самое соответствие. Отозвал, вернул согласие, отозвал снова —
// две могилы, и это правильнее одной.
//
// В audit_log не пишется ничего. Запись «этот человек обезличил свои заметки»
// вместе с временем переноса и есть соответствие, отсутствие которого делает
// обезличивание обезличиванием.
func anonymizeOwnNotes(ctx context.Context, q querier, userID int64) (int, error) {
	// Спрашиваем ДО заведения могилы: без этого у каждого, кто отозвал согласие
	// не написав ни заметки, в users оставалась бы пустая могила.
	var any bool
	if err := q.QueryRow(ctx,
		`SELECT exists(SELECT 1 FROM notes WHERE author_id = $1)`, userID).Scan(&any); err != nil {
		return 0, wrapf(err, "обезличивание заметок %d", userID)
	}
	if !any {
		return 0, nil
	}
	grave, err := newGrave(ctx, q)
	if err != nil {
		return 0, err
	}
	// Статусы не трогаются вовсе — значит не трогается и денормализованный
	// notes.comment_count: видимость этой операцией не меняется ни у чего.
	rows, err := q.Query(ctx,
		`UPDATE notes SET author_id = $2 WHERE author_id = $1 RETURNING id`, userID, grave)
	if err != nil {
		return 0, wrapf(err, "обезличивание заметок %d", userID)
	}
	var ids []int64
	...
	// Карточки проверки едут следом: иначе очередь модератора продолжала бы
	// показывать, чья это заметка. Комментарии в очереди остаются его — они и
	// на странице остаются его.
	if _, err := q.Exec(ctx, `
		UPDATE moderation_queue SET author_id = $2
		 WHERE author_id = $1 AND subject_kind = $3`, userID, grave, SubjectNote); err != nil {
		return 0, wrapf(err, "обезличивание заметок %d", userID)
	}
	// Шина. Чистится ровно то, что связывает ЕГО с этими заметками, а не всё
	// подряд: события про реплики в его теме принадлежат другим людям, и снеся
	// их, мы отняли бы у посторонних поводы, о которых они не просили. Отсюда
	// comment_id IS NULL — событие о самой заметке, а не о реплике в ней.
	//
	// Поводы ему самому снимаются шире, по всем событиям этих заметок: строка
	// «повод о заметке N адресован X» и есть свидетельство, что заметка его.
	//
	// Индекса по events.note_id нет, и это перебор — но таблица живёт 90 суток
	// (KeepEvents) и растёт со скоростью разговора, а не архива.
	if _, err := q.Exec(ctx, `
		DELETE FROM notifications
		 WHERE user_id = $1
		   AND event_id IN (SELECT id FROM events WHERE note_id = ANY($2))`,
		userID, ids); err != nil {
		return 0, wrapf(err, "обезличивание заметок %d", userID)
	}
	if _, err := q.Exec(ctx, `
		DELETE FROM events
		 WHERE note_id = ANY($2) AND comment_id IS NULL
		   AND (actor_id = $1 OR subject_user_id = $1)`, userID, ids); err != nil {
		return 0, wrapf(err, "обезличивание заметок %d", userID)
	}
	// Будущие поводы по этим заметкам отсекутся сами: правила раздачи требуют
	// от получателя kind = member и anonymized_at IS NULL, а могила не то и не
	// другое.
	return len(ids), nil
}
