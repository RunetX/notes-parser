...
// разговору не придёт.
//
// ДВОЙНИКОВ в карте нет (`synth_of IS NULL`): смежное обсуждение — не
// самостоятельная запись, а приложение к чужой заметке, и тело у него служебное
// («здесь о соседней заметке говорят жители»). Звать на него поисковик значит
// вести человека к странице, за которой стоит не текст, а машина; на саму
// заметку он придёт и так, а ссылка с неё никуда не денется.
//
...
const sitemapQuery = `
	SELECT id, greatest(published_at, coalesce(last_comment_at, published_at))
	  FROM notes WHERE status = 0 AND synth_of IS NULL
	 ORDER BY id
	 LIMIT $1 OFFSET $2`
