...
// АНОНИМНАЯ ЗАМЕТКА УХОДИТ, И УХОДИТ АНОНИМНОЙ (правлено 02.09.2026 по вопросу
// владельца «почему заметка не улетела из Зазеркалья на НГС»).
//
// До этого дня она молчала, и довод стоял неверный: «на НГС её пришлось бы
// публиковать от имени автора». Сайт принимает анонимную заметку сам
// (action_note[hideme]=1), love.Client.PostNote берёт признак параметром, а
// РюмкинЪ публикует так с самого начала. Обещание, данное человеку здесь, там не
// нарушается — маску ставит НГС.
//
// Признак обязан ДОЕХАТЬ до службы, поэтому проверяется он у самой работы, а не
// только наличие строки в очереди: отправь мы анонимную заметку с hideme=0 — и
// имя человека встало бы под текстом, который он публиковал скрытно. Починить
// это было бы уже нечем.
func TestАнонимнаяЗаметкаУходитАнонимной(t *testing.T) {
	p := testPlatform(t)
	ctx := context.Background()
	author := mustNGSMember(t, p, 1493279, "Рио")
	...
	if n := outboxCount(t, p); n != 2 {
		t.Fatalf("анонимная заметка не встала в очередь: строк %d, ожидалось 2", n)
	}
	...
	if len(jobs) != 2 {
		t.Fatalf("выдано строк %d, ожидалось 2", len(jobs))
	}
	for _, j := range jobs {
		want := j.Body == "тайно"
		if j.Anonymous != want {
			t.Errorf("у заметки %q анонимность %v, а ожидалась %v", j.Body, j.Anonymous, want)
		}
	}
}

// ЭХО УЗНАЁТ СВОЮ АНОНИМНУЮ ЗАМЕТКУ — БЕЗ АВТОРА, потому что автора у неё на
// сайте нет вовсе. Это и была настоящая причина, по которой анонимные заметки
// не отпускали: не опознав своё, зеркало принесло бы их обратно второй строкой
// в ленту и постами в оба канала.
//
// Невод здесь о двух нитях (текст да сутки) вместо трёх, и цена названа: чужая
// анонимка с тем же началом внутри окна может быть принята за нашу. Больше
// отправленного он не погасит — ngs_id проставляется строке ровно один раз.
func TestЭхоУзнаётАнонимнуюЗаметкуБезАвтора(t *testing.T) {
	p := testPlatform(t)
	ctx := context.Background()
	author := mustNGSMember(t, p, 1493279, "Рио")
	if err := p.SetNGSSend(ctx, author, true); err != nil {
	...
	const body = "Написал тайно, а прочитать хочу обеими половинами сразу."
	if _, err := p.CreateNote(ctx, NewNote{AuthorID: author, Anonymous: true, Body: body}); err != nil {
		t.Fatalf("заметка: %v", err)
	}
	if _, err := p.NextNGSJobs(ctx, 5); err != nil {
		t.Fatalf("выдача: %v", err)
	}
	// Зеркало спрашивает ИМЕННО ТАК: у анонимной записи сайта автора нет.
	own, err := p.ClaimNGSEcho(ctx, NGSNote, 0, 0, body, "313200", time.Now())
	if err != nil {
		t.Fatalf("опознание: %v", err)
	}
	if !own {
		t.Fatal("своя анонимная заметка не опознана — вернулась бы дублем в ленту и в оба канала")
	}
	// Второй раз та же запись узнаётся снова: память зеркала лежит в SQLite, и
	// не записавшаяся там отметка иначе означала бы дубль на следующем такте.
	if own, err := p.ClaimNGSEcho(ctx, NGSNote, 0, 0, body, "313200", time.Now()); err != nil || !own {
		t.Fatalf("повторное опознание: %v, %v", own, err)
	}
	// А чужая анонимка с другим текстом нашей не считается.
	if own, err := p.ClaimNGSEcho(ctx, NGSNote, 0, 0, "совсем про другое", "313201", time.Now()); err != nil || own {
		t.Errorf("чужая анонимка принята за свою: %v, %v", own, err)
	}
}
