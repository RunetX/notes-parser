...
type Author struct {
	...
	// «не показываем», и у участника здесь всегда ноль: подписанные им согласия
	// обещают, что поля анкеты, кроме ника, аватара и пола, не показываются, —
	// поэтому колонка у него и не заполняется вовсе (0027_users_age.sql).
	Age int
...
type MirroredAuthor struct {
	...
	Nick      string
	AvatarURL string
	// Age — возраст из alt аватара НГС («Ник, 43 года»), ноль — не знаем.
	// Доезжает сюда ДАРОМ: зеркало разбирает его с первого дня и с тех же пор
	// печатает шапкой реплики в Telegram и MAX. Ложится он только на ТЕНЬ — тем
	// же условием ensureShadow, которым зеркало не смеет переписать ник
	// вошедшему (см. 0027_users_age.sql).
	Age int
}
...
type NewComment struct {
	...
	Body      string
	ReplyToID int64
	// Quiz — варианты ответа, если эта реплика задаёт пятничный вопрос
	// (эпик J, platform/quiz.go). Поле здесь, а не в отдельном вызове, потому
	// что варианты кладутся ТОЙ ЖЕ транзакцией, что и сама реплика: вопрос без
	// вариантов — это то же состояние, что «опубликовано, но в очередь не
	// попало», только видное всем и сразу.
	Quiz *NewQuiz
}
