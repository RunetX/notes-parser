...
const (
	...
	// 1 и 3 — занятые номера, а не пропуск. 1 значил «скрыл автор» и ставился
	// отзывом согласия, пока тот прятал публикации; с 25.08.2026 отзыв
	// обезличивает заметки, ничего не пряча (migrate 0016 вернула на вид всё,
	// что было скрыто им). 3 значил «обезличено» и не ставился никогда: у
	// обезличивания нет своей видимости — оно меняет автора, а не статус.
	// Переиспользовать номера нельзя: смысл строки в старых бэкапах от этого
	// молча изменится.
)
...
// CanAdmin — смотрящий вправе то, чего нет даже у модератора: раздавать права и
// впускать людей. Оба действия размножают доступ, поэтому и отделены — модератор
// решает про СЛОВА, администратор про то, кто их здесь пишет.
func (v Viewer) CanAdmin() bool { return v.Role >= RoleAdmin }
...
type Author struct {
	...
	Shadow bool
	// Persona — реплики этого автора пишет машина (users.persona, эпик
	// «народ»). Показу это нужно ровно за одним: страница жителя открыта
	// ГОСТЮ, и его имя обязано быть ссылкой там же, где имя живого участника
	// остаётся текстом (см. web.nickOf). Второго признака «выдуманный» рядом
	// не заводится — тот же довод, что у Shadow: различие показывается на
	// АВТОРЕ и ровно одно.
	Persona bool
	// System — это САМА ПЛОЩАДКА (users.kind = KindService): под её анкетой
	// выходит недельный выпуск. Показу это нужно затем, что в колонке автора у
	// такой записи стоит ЗНАК площадки, а не лицо, — тот же вид, что у двойника
	// (parts/sitemark.gohtml): силуэт по умолчанию говорил бы «человек, у
	// которого нет фото», а человека здесь нет вовсе.
	//
	// У РЕПЛИКИ признака нет: площадка заводит записи, а не разговаривает, —
	// вида «системный комментарий» не существует, и заводить его ради колонки,
	// которая всегда false, значило бы платить за него на самом горячем запросе
	// площадки.
	System bool
}
...
type NoteView struct {
	...
	// Pinned — заметка закреплена наверху ленты. Показывается ВСЕМ и без
	// объяснения: закрепление это не свойство заметки, а решение площадки, и
	// читателю важно понимать, почему старая запись стоит выше свежих.
	Pinned bool
	// Stage — заметка-песочница: разговор в ней ведут жители (эпик «народ»), а
	// участник её читает. Показывается ВСЕМ и всегда: страница, в которой нельзя
	// ответить, обязана сказать почему, — а метки этой на НГС не было, значит по
	// правилу Ш5з она объясняет себя сама.
	Stage bool
	// SynthOf — у ДВОЙНИКА: чьё смежное обсуждение здесь ведут. Ноль у всех
	// остальных заметок.
	SynthOf int64
}
...
type NewNote struct {
	...
	Anonymous bool
	Body      string
	// Image — иллюстрация, УЖЕ ЛЕЖАЩАЯ в хранилище: байты на диске, строка в
	// media заведена. Ядро её только привязывает, и привязывает той же
	// транзакцией, что и саму заметку.
	//
	// Файлов ядро не трогает вовсе — ни кладёт, ни удаляет. Это граница, за
	// которой нет состояния «заметка опубликована, а картинка потерялась»: к
	// моменту вызова картинка либо есть на диске, либо публикации не будет.
	Image *Media
	// Stage — заметка-ПЕСОЧНИЦА: читают все, комментируют только жители (эпик
	// «народ») и администратор. Ставить её вправе тот же круг — проверка в
	// CreateNote, а не в форме.
	Stage bool
	// SynthOf — номер заметки, чьё СМЕЖНОЕ обсуждение здесь ведут. Заполняется
	// только двойником (CreateSynthThreadAsAdmin) и обязывает к Stage: двойник
	// без песочницы был бы обычной заметкой, в которой почему-то говорят машины.
	SynthOf int64
	// KeepHere — эту заметку на НГС не уносить, даже если у автора стоит
	// галочка. Ставит её ровно один вызывающий — PublishNGSDraftHere, то есть
	// откат черновика, который сайт не взял за три попытки. Без него заметка
	// встала бы в ngs_outbox и поехала бы туда ещё раз, четвёртой попыткой в
	// обход потолка, а удайся она — текст вышел бы и здесь, и там.
	KeepHere bool
}

// NoteEdit — правка своей заметки в окне.
//
// Одним значением, а не тремя аргументами, ровно из-за DropImage: снятие
// картинки обязано пройти ТОЙ ЖЕ транзакцией, что и текст. Разведи их на две —
// и отказ второй половины оставит человека с закрытым навсегда окном (edited_at
// проставлен, править можно один раз) и с картинкой, которую он снимал. Это не
// редкий случай, а обычный обрыв связи.
