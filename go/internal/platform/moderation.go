...
//
// ВТОРОЙ: ссора, колкость, троллинг, провокация и скрытая отсылка — это ЖАНР
// раздела, а не нарушение. Модель, настроенную на «токсичность», нельзя пускать
// сюда вовсе: она выкосит саму площадку и первыми — самых заметных её людей.
// Граница проведена владельцем 23.08.2026 и проходит НЕ по резкости, а по двум
// вещам, которые правила запрещают прямо: нецензурная брань (в том числе
// замаскированная) и прямое оскорбление с переходом на личности. Троллить и
// провоцировать здесь можно, материться и обзывать человека — нет. Поэтому
...
const (
	...
	CatProfanity = "profanity" // нецензурная брань, в том числе замаскированная
	CatInsult    = "insult"    // прямое оскорбление с переходом на личности
...
var categoryTitles = map[string]string{
	...
	CatProfanity: "нецензурная брань",
	CatInsult:    "оскорбление участника",
...
//
// Чего в нём нет намеренно: «токсичность», offtopic, спор о политике, троллинг
// и провокация — это жанр раздела.
//
// **`profanity` здесь ЕСТЬ, а `insult` нет** (решение владельца 23.08.2026), и
// разница не в тяжести, а в том, кто способен ошибиться. Мат — факт написания:
// его находит код по короткому списку корней (см. platmod/profanity.go), решение
// воспроизводимо и объяснимо цитатой, поэтому автомату можно доверить и скрытие.
// Оскорбление — суждение о том, к кому обращено и с каким намерением; его
// отличает от той же колкости только адрес, ошибиться тут легко, и цена ошибки —
// молча снятая реплика. Поэтому insult зовёт ЧЕЛОВЕКА.
//
// **`spam` отсюда УБРАН** (решение владельца 23.08.2026) — по замеру, а не из
// осторожности. На 738 живых строках он не нашёл ни одной настоящей рекламы,
// зато успел назвать спамом ссылку на нашу же справку `t3h.ru/help` («ссылка на
// сторонний сайт»), ответ администратора про будущие приложения и реплику
// «сделал доброе дело женщине, или бескорыстно или назови свою цену за работу».
// Причина глубже промпта: спам отличает от разговора НАМЕРЕНИЕ, а не форма, и
// ссылка сама по себе улика никакая. Молодое сообщество ссылками на себя и
// живёт, поэтому цена ошибки здесь выше обычного.
...
func AutoCategories() []string {
	return []string{CatSpam, CatPII, CatThreat, CatDrugs, CatSuicide,
		CatExtremism, CatPorn, CatFlood, CatProfanity, CatInsult, CatOther}
}
...
const (
	...
	ActionExport   = "export"
	// ActionMailExport — ВЫДАЧА ПЕРЕПИСКИ уполномоченному органу (эпик L).
	// Отдельным действием от ActionExport, и это не педантизм: там выгрузка
	// СУБЪЕКТУ его же данных по 152-ФЗ, здесь — исполнение запроса извне по
	// 149-ФЗ. Одной строкой на двоих журнал перестал бы отвечать на
	// единственный вопрос, который к нему придут задавать: кому и когда отдали
	// чужие письма.
	ActionMailExport = "mail_export"
...
// SubjectInvite — четвёртый вид, и только для журнала: приглашение. Своего
// идентификатора у него нет (ключ строки — хеш кода, а хеш не показывают), и
// номером в журнале стоит ТОТ, К КОМУ приглашение привязано; ноль означает
// «ни к кому», то есть пришедшего заведут с нуля.
const SubjectInvite = "invite"
