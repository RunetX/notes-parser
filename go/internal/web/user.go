...
func (s *Server) handleUser(w http.ResponseWriter, r *http.Request) {
	...
	// Гостю открыты только ЖИТЕЛИ, и «такого нет» отвечается ему ТАК ЖЕ, как
	// «есть, но живой»: разведи мы эти два ответа, перебор номеров сказал бы
	// постороннему, какие анкеты на площадке заведены. Вход, а не «нет такой
	// страницы», потому что страница ЕСТЬ и войдя человек её увидит.
	if !signedIn && (err != nil || !member.Persona) {
