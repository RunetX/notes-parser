...
type page struct {
	...
	Bell   bool
	Unread int
	// Letters — непрочитанных писем, и показывается это число ПУНКТОМ МЕНЮ, а не
	...
	// «Letters > 0», по тому же доводу, что у Bell: пункт меню стоит и пустым,
	// иначе он появлялся бы и пропадал на каждом переходе.
	MailOn bool
