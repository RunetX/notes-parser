...
type page struct {
	...
	Bell   bool
	Unread int
	// MailUnread — непрочитанных писем, и показывается это число ПУНКТОМ МЕНЮ, а не
	// пятым значком в шапке: пятый элемент уже ломал телефон 23.08.2026 (довод
	// стоит в base.gohtml), а о новом письме и так говорит колокольчик — повод
	// «вам написали» идёт шиной наравне с остальными.
	//
	// Ноль означает и «переписка выключена», и «непрочитанного нет»: и там и там
	// числа рядом с пунктом не рисуется, а сам пункт держит MailOn.
	//
	// Имя СОСТАВНОЕ намеренно: поле страницы, названное так же, как поле
	// каркаса, затеняет его в шаблоне — и каркас печатает чужое. Простые
	// «Letters» здесь уже стоили меню, полного писем (12.09.2026, см.
	// page_shadow_test.go).
	MailUnread int
	// MailOn — переписка включена и человек вошёл. Отдельным полем, а не
	// «MailUnread > 0», по тому же доводу, что у Bell: пункт меню стоит и
	// пустым, иначе он появлялся бы и пропадал на каждом переходе.
	MailOn bool
	// InThread и Thread — счётчик реплик той заметки, которую человек читает:
	// показывать ли его вовсе и какое число в нём стоит. Полей ДВА, а не одно,
	// по тому же доводу, что у колокольчика выше: «это не страница заметки» и
	// «реплик пока ноль» — разные вопросы, а одно число отвечало на оба сразу.
	// Ценой была не вёрстка: подкрутить счётчик живой добор умеет, а создать
	// его нет, — и в заметке, открытой пустой (песочницу и двойника только
	// такими и открывают), число в шапке не появлялось до перезагрузки, сколько
	// бы реплик в неё ни пришло. Поля ОБЩЕЙ части, а не страницы заметки, потому
	// что рисует счётчик шапка: она одна на все страницы, и лезть из неё в поля
	// конкретной значило бы уронить все остальные.
	InThread bool
	Thread   int
	// Faces — мордолента: лица жителей полосой НАД карточкой (faces.go). Поле
	// ОБЩЕЙ части, а не ленты, по той же причине, что и Thread: рисует полосу
	// КАРКАС — на НГС она стоит над блоком содержимого, а не внутри него, — а
	// лезть из каркаса в поля одной страницы значит уронить все остальные.
	// Заполняет его только лента и только на первой странице; пусто — полосы
	// нет вовсе.
...
func (s *Server) newPage(r *http.Request, title string) page {
	...
	if u, ok := s.me(r); ok {
		...
		p.MailOn = s.mail != nil
		if p.MailOn {
			// Второй индексный запрос на страницу вошедшего, и плата названа
			// заранее: он идёт по частичному индексу mail_sides_unread, то есть
			// по строкам, у которых непрочитанное есть. Отказ счётчика страницу
			// не рушит — как и у колокольчика: непрочитанное не то, ради чего
			// стоит отдавать человеку 500.
			n, err := s.mail.UnreadMail(r.Context(), u.ID)
			if err != nil {
				s.log.Warn("счётчик писем", "user", u.ID, "err", err)
			}
			p.MailUnread = n
		}
	}
