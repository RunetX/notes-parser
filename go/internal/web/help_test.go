...
func TestКонтактыПоказываютсяТолькоНастроенные(t *testing.T) {
	...
	if strings.Contains(body, "Владелец в MAX") {
		t.Error("показан личный MAX владельца, которого в настройках нет")
	}
}

// ЛИЧНЫЙ MAX ВЛАДЕЛЬЦА — своя строка и своё поле (просьба владельца 12.09.2026).
// Оно НЕ группа и не бот: с закрытием чтения ссылка на страницу владельца ведёт
// не вошедшего в редирект на вход, а пишут ему чаще всего именно те, кто войти
// не смог. Поэтому раздел «Как связаться» обязан показываться и тогда, когда из
// контактов настроено ОДНО это поле, — иначе у площадки за воротами не осталось
// бы ни одного живого адреса.
func TestЛичныйMAXВладельцаСтоитВКонтактах(t *testing.T) {
	const link = "https://max.ru/id00000000"
	body := helpBody(t, "/help", Config{MembersOnly: true, Contacts: Contacts{OwnerMAX: link}})
	if !strings.Contains(body, "Как связаться") {
		t.Fatal("одного личного адреса не хватило, чтобы показать контакты")
	}
	if !strings.Contains(body, `href="`+link+`"`) {
		t.Error("на код нельзя нажать: с телефона свой же экран камерой не снять")
	}
	// А СТРОКОЙ адрес не печатается вовсе (решение владельца): семьдесят шесть
	// знаков токена не прочесть и не перепечатать, читать их некому.
	if strings.Contains(body, ">"+link+"<") {
		t.Error("токен напечатан текстом — его и просили заменить кодом")
	}
	// Сам код: он обязан быть КАРТИНКОЙ В РАЗМЕТКЕ и обязан зависеть от адреса.
	// Второе важнее первого: заглушка, рисующая всегда одно и то же, прошла бы
	// любую проверку на «есть svg», а с экрана вела бы не туда.
	if !strings.Contains(body, `role="img"`) || !strings.Contains(body, "crispEdges") {
		t.Error("кода на странице нет")
	}
	other := helpBody(t, "/help", Config{Contacts: Contacts{OwnerMAX: "https://max.ru/u/другой"}})
	if qrPath(t, body) == qrPath(t, other) {
		t.Error("код не зависит от адреса — картинка нарисована мимо настройки")
	}

	// Кривой адрес гасится тем же правилом, что и остальные ссылки: «max.ru/…»
	// без схемы браузер прочтёт относительным путём и уведёт на t3h.ru/max.ru/…
	dim := checkContacts(Contacts{OwnerMAX: "max.ru/id00000000"}, quietLog())
	if dim.OwnerMAX != "" {
		t.Errorf("ссылка без схемы не погашена: %q", dim.OwnerMAX)
	}
}

// qrPath — рисунок кода со страницы: содержимое единственного <path>.
//
// Сравнивать страницы целиком нельзя — у них разный CSRF, разное время и разный
// адрес в href; вопрос же ровно один: тот ли КОД нарисован.
func qrPath(t *testing.T, body string) string {
	t.Helper()
	const head = `<path fill="#000" d="`
	i := strings.Index(body, head)
	if i < 0 {
		t.Fatal("на странице нет рисунка кода")
	}
	rest := body[i+len(head):]
	j := strings.IndexByte(rest, '"')
	if j < 0 {
		t.Fatal("рисунок кода не закрыт")
	}
	return rest[:j]
}

// Сам кодировщик: пустой адрес не даёт пустой картинки, а настоящий укладывается
// в код, который ЧИТАЕТСЯ. Читаемость проверяется не здесь, а на стенде: код,
// нарисованный этой же функцией для боевого адреса, скормлен постороннему
// декодеру (OpenCV) и вернул ровно исходную строку. В тесте остаётся то, что
// проверяемо без камеры: рисунок есть, он квадратный и меняется вместе с адресом.
func TestКодСтроитсяИзАдреса(t *testing.T) {
	empty, err := qrSVG("", qrSide)
	if err != nil || empty != "" {
		t.Errorf("пустой адрес дал картинку: %q, %v", empty, err)
	}
	one, err := qrSVG("https://max.ru/u/первый", qrSide)
	if err != nil {
		t.Fatal(err)
	}
	two, err := qrSVG("https://max.ru/u/второй", qrSide)
	if err != nil {
		t.Fatal(err)
	}
	if one == two {
		t.Error("разные адреса дали одинаковый код")
	}
	if !strings.Contains(string(one), `viewBox="0 0 `) || !strings.Contains(string(one), `width="168"`) {
		t.Errorf("картинка кода не похожа на картинку: %s", one)
	}
}
