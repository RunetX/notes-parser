...
// Реакции заметки стоят В СТРОКЕ ДАТЫ, а не отдельной строкой над ней (жалоба
// владельца 11.09.2026: «целая строка на „+“»). Под заметкой без реакций в этом
// ряду стоит один-единственный плюс, и целая строка под него — строка ни о чём.
//
// Проверяется именно ВЛОЖЕННОСТЬ, а не «реакции есть»: подвал заметки — блок,
// поэтому сосед, вынесенный наружу, начинает новую строку сам, сколько бы места
// ни оставалось, — и увидеть это можно только глазом на странице. Тот же тест и
// тот же довод, что у метки происхождения в строке даты реплики.
func TestNoteReactionsSitInsideTheDateLine(t *testing.T) {
	h, _, token := writeServer(t, reactStore())

	body := do(h, as(httptest.NewRequest("GET", "/n/312811", nil), token)).Body.String()
	i := strings.Index(body, `<div class="nfoot">`)
	if i < 0 {
		t.Fatal("у заметки нет подвала со временем")
	}
	// Внутри подвала своих <div> нет: метка, дата и «исправлено» — это span'ы,
	// а реакции участнику рисуются формой. Значит первый же </div> и есть его
	// собственный конец.
	line := body[i : i+strings.Index(body[i:], "</div>")]
	if !strings.Contains(line, `class="rx"`) {
		t.Error("реакции выпали из строки даты — «+» снова займёт целую строку")
	}
	if !strings.Contains(line, `class="date`) {
		t.Fatal("в подвале нет даты — тест смотрит не туда")
	}
}
