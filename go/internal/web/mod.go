...
// исчерпывающий ответ на вопрос «что площадка позволяет модератору».
//
// Правка чужого текста в нём ОДНА — EditNoteAsAdmin, и она администраторская:
// модератор чужих слов не переписывает, он их скрывает. Видно это с одного
// взгляда, и так и задумано — список читается как список прав.
//
...
type Moderator interface {
	...
	// EditNoteAsAdmin — поправить текст НАТИВНОЙ заметки. Единственное место,
	// где чужой текст меняется, а не скрывается: у площадки есть свои заметки
...
type modAct struct {
	...
	AdminShot bool
	// AdminStage — заметку ещё можно отдать ЖИТЕЛЯМ (эпик «народ»): она
	// молчит, а значит ядро согласится. Ссылка та же самая — песочница живёт
	// галочкой на той же форме правки, — но подпись у неё меняется, и вот
	// почему это не косметика.
	//
	// Дверь была подписана по своему прежнему содержимому («Поправить» у
	// нативной, «Картинка» у зеркальной), а за ней появилось второе действие,
	// которого по вывеске не найти: 30.08.2026 владелец смотрел на страницу
	// заметки 313128 и не увидел, как запустить жителей, — ссылка называлась
	// «Картинка». Форму называют по тому, ЗАЧЕМ в неё идут, а не по полям.
	//
	// Из двух возможностей называем песочницу, а не текст, потому что она
	// СРОЧНАЯ: правило «только пока никто не сказал ни слова» действует до
	// первой реплики, а текст правится и завтра. Цена размена записана честно:
...
func modFeedNote(d noteItemData) modAct {
	return modAct{
		...
		AdminStage:  d.Page.CanEdit && d.Note.CommentCount == 0,
