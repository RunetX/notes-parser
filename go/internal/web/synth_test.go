...
// Длинная цитата сворачивается и на СТРАНИЦЕ двойника (01.09.2026): приходят
// сюда за разговором, а цитата ему предисловие — простыня в предисловии
// отодвигает реплики на несколько экранов вниз. Текст при этом, как везде,
// отдан целиком: свёрнут показ.
func TestДлиннаяЦитатаСворачиваетсяИНаСтраницеДвойника(t *testing.T) {
	st := twinStore()
	o := st.origins[100000000030]
	o.Body = strings.Repeat("длинный оригинал. ", 200) // ~3600 знаков
	st.origins[100000000030] = o
	body := do(openServer(t, st), guest(t, "GET", "/n/100000000030")).Body.String()

	if !strings.Contains(body, `class="text clip"`) || !strings.Contains(body, `for="exq100000000030"`) {
		t.Error("цитату на странице двойника нечем развернуть")
	}
	if strings.Count(body, "длинный оригинал") < 200 {
		t.Error("текст свёрнутой цитаты обрезан на сервере")
	}
}
