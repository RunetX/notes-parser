...
type videoCard struct {
	Provider string // подпись: «YouTube», «Rutube», «VK Видео»
...
//
// Российские рядом с YouTube не для полноты списка: они не замедлены, и вопроса
// о вывозе данных не задают вовсе — на них карточка работает лучше, чем на
// первом.
...
var providers = []provider{{
	...
	name:    "VK Видео",
	dir:     "vk",
	hosts:   []string{"vk.com", "www.vk.com", "m.vk.com", "vkvideo.ru", "www.vkvideo.ru"},
	id:      regexp.MustCompile(`^video-?[0-9]+_[0-9]+$`),
	videoID: func(u *url.URL) string { return firstSegment(strings.Trim(u.Path, "/")) },
	oembed: func(page string) string {
		return "https://vk.com/oembed?format=json&url=" + url.QueryEscape(page)
	},
}}
