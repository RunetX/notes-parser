...
func (s *Server) handleFresh(w http.ResponseWriter, r *http.Request) {
	...
	if len(rows) > 0 {
		...
		// ПЯТНИЧНЫЙ ВОПРОС — здесь же, и это НЕ то же самое, что реакции.
		//
		// Реакции у только что появившейся реплики пусты у всех, потому что их
		// ещё некому было нажать; варианты же кладутся ТОЙ ЖЕ транзакцией, что и
		// сама реплика, — то есть у вопроса они есть с рождения. Без этой строки
		// вопрос, приехавший живым добором, вставал на открытую страницу голым
		// текстом, а варианты появлялись только после обновления: ровно то, чего
		// добор и должен избежать. Поймано владельцем на первом же боевом вечере
		// 11.09.2026 — четвёртый вопрос выпуска.
		//
		// Спрашивается только когда порция НЕ ПУСТА, а пустых полосе большинство:
		// в обычном треде это один запрос по индексу, возвращающий ноль строк.
		if quiz, err := s.st.NoteQuiz(ctx, me.ID, id); err != nil {
			s.log.Warn("вопросы заметки", "заметка", id, "err", err)
		} else if len(quiz) > 0 {
			p.Quiz = quiz
			p.QuizGuest = s.guestQuizOf(r, quiz)
		}
	}
