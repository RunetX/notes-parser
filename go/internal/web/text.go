...
// longBodyRunes — с какого объёма реплика показывается НАЧАЛОМ, а остальное по
// кнопке (решение владельца 23.08.2026: «простыня выглядит не очень»). Полторы
// тысячи знаков — это примерно экран текста на десктопе и несколько экранов на
// телефоне; обычная реплика в три-пять строк порога не достаёт.
//
// Ограничение стоит ПОКАЗОМ, а не запретом писать, и это не полумера: в базе
// 10,7 млн зеркальных реплик за тринадцать лет, переписать их нельзя, а запрет
// на длину чинил бы только будущее, оставив ровно ту страницу, с которой пришла
// жалоба. Санитарный потолок (platform.MaxBodyRunes) при этом остаётся на месте
// и решает другую задачу — чтобы мегабайт не стал строкой в базе.
const longBodyRunes = 1500

// isLongBody — показывать ли реплику свёрнутой. Считает знаки, а не строки:
// строк на экране столько, сколько их нарисовала ширина колонки, и на телефоне
// их втрое больше, чем на десктопе.
func isLongBody(s string) bool { return utf8.RuneCountInString(s) > longBodyRunes }
...
func eraOf(id int64, t time.Time) era {
	...
	// Форм у смайла ДВЕ (см. smileyRe), и старшая — «~popcorn~»: ею ставили
	// смайлы в заметках все двенадцать лет, пока сайт их рисовал, — а в своём
	// тексте она разбирается по тому же доводу, что и скобки: знаки эти у
	// переехавших в пальцах.
