...
// aboutBodyHTML — рассказ человека о себе (эпик M).
//
// Эпоха задана НАТИВНОЙ прямо, а не выведена из номера, и это не мелочь.
// eraOf решает по platform.IsNative(id), а у живого участника id — это номер
// его анкеты на НГС, то есть НИЖЕ границы нативной полосы: спроси мы эпоху по
// нему, написанное ЗДЕСЬ объявилось бы чужим текстом, и скобки со смайлами
// остались бы буквами у всех, кроме вошедших по приглашению. Рубеж 2014 года —
// правило про ЧУЖОЙ текст, а этот человек пишет у нас и панель смайлов видит
// тут же под полем.
func aboutBodyHTML(text string) template.HTML {
	return renderBody("", text, era{markup: true, smiles: true, links: linkOwn})
}
