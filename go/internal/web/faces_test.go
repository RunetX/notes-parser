...
// содержимого, внутри один ряд фотографий и НИ ОДНОЙ подписи: имя живёт
// в alt и всплывает при наведении (там qtip, у нас title — он работает без JS).
//
// Размер лица записанная страница НЕ называет — там стои́т имя файла
// (`..._100_100_c.jpg`), то есть размер ИСХОДНИКА. Показ задаёт таблица стилей,
// и он другой; см. TestЛицоВПолосеМеньшеАватараАвтора.
...
// ЛИЦО В ПОЛОСЕ МЕНЬШЕ АВАТАРА АВТОРА — замечание владельца 31.08.2026 («в
// оригинальной мордоленте аватары меньше аватаров авторов заметок»), и оно
// подтвердилось замером живого index.css НГС:
//
//	.lv-top-tape__link    { padding: 5px; height: 70px; width: 70px }
//	.lv-top-tape__userpic { width: 100%; height: 100%; border-radius: 3px }
//
// против 100×100 у колонки автора. Стояло у нас 100 — размер САМОГО ФАЙЛА,
// который называет разметка записанной страницы; размер показа она не называет
// вовсе, и первого без второго оказалось мало.
//
// Проверяется ОТНОШЕНИЕ, а не только число: смысл замечания в том, что полоса
// легче карточки, — и тест обязан ловить возврат к равенству, каким бы числом
// его ни сделали.
func TestЛицоВПолосеМеньшеАватараАвтора(t *testing.T) {
	css := cssText(t)
	band, author := cssRule(t, css, ".faces .ava {"), cssRule(t, css, ".ava {")
	if !strings.Contains(band, "width: 70px") || !strings.Contains(band, "height: 70px") {
		t.Errorf("лицо в полосе не 70×70: %s", band)
	}
	if !strings.Contains(author, "width: 100px") {
		t.Errorf("аватар автора перестал быть 100px: %s", author)
	}
	// Атрибуты картинки обязаны совпадать с показом, иначе полоса прыгает,
	// пока фотографии грузятся, — ровно то, ради чего они и проставлены.
	body := do(openServer(t, facesStore()), guest(t, "GET", "/")).Body.String()
	i := strings.Index(body, `class="faceband"`)
	if i < 0 {
		t.Fatal("полосы нет вовсе")
	}
	if tape := body[i:strings.Index(body, `<main class="main"`)]; !strings.Contains(tape, `width="70" height="70"`) {
		t.Error("у лиц в полосе атрибуты размера разошлись с показом")
	}
}
