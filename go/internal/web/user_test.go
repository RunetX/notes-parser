...
// Своя страница участника называет, где её правят. Сама она форм не показывает
// вовсе: показ и правка стоят рядом на «Моей странице» (12.09.2026), а сюда
// человек попадает по своему имени из чужого треда — и должен узнать дорогу.
func TestСвояСтраницаНазываетГдеЕёПравят(t *testing.T) {
	st := profileStore()
	st.profile.ID = testProfileID
	h, _, token := profileServer(t, st, platform.RoleUser)
	body := do(h, as(guest(t, "GET", "/u/"+itoa64(testProfileID)), token)).Body.String()
	...
	if !strings.Contains(body, `href="/me"`) {
		t.Errorf("на своей странице нет дороги к «Моей странице»:\n%s", tailOf(body))
	}
	// И форм тут нет: они там, где виден их результат.
	if strings.Contains(body, `name="bio"`) {
		t.Error("форма правки завелась на чужой по устройству странице")
	}
}
