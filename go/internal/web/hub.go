...
// подключается она type-assertion'ом от Events (тот же приём, что у dmbot.SiteProfile), и её
...
// LiveMover — способность сказать, в каких заметках ПЕРЕЕХАЛИ ветки.
//
// Третий источник сигнала рядом с фактами и поводами, и заведён он не для
// полноты: обход мобильного дерева (platsink.ReplyScanner) заменяет угаданное
// зеркалом ребро настоящим, и ветка на открытой странице уезжает на своё место.
// Отметка moved_at и заголовок X-Fresh-Moved для этого уже есть — не было
// ПОВОДА сходить за ними: добор идёт по сигналу, а сигналы рождались только из
// events, куда переезд не пишется и писать его незачем («ветка встала на место»
// не то, о чём говорят участнику).
//
// Оттого стоило это ровно одной жалобы владельца (24.08.2026): разговор в треде
// затихал сразу после ответа, следующей реплики не случалось, сигнала не
// приходило — и человек до перезагрузки смотрел на ветку, выросшую по догадке,
// хотя в базе она уже стояла правильно. Продолжись разговор — переезд приехал бы
// ЗАОДНО со следующей репликой, и потому это выглядело как «иногда».
//
// Необязательная, как и остальные: нет её — переезды доходят по-старому, вместе
// со следующей репликой.
type LiveMover interface {
	MovedNotesSince(ctx context.Context, notes []int64, after time.Time) (map[int64]time.Time, error)
}
...
type hub struct {
	...
	events int64
	pokes  int64
	// moves — граница переездов. Время, а не id: переезд правит СТАРУЮ строку,
	// и её номер о том, что она сдвинулась, не говорит ничего.
	moves  time.Time
	inited bool
...
func (h *hub) start(ctx context.Context) bool {
	...
	// Переезды тоже с этого мгновения: прошлое не рассылается, а страница,
	// открытая позже, и так рисуется по нынешнему дереву.
	h.moves = time.Now()
	h.mu.Unlock()
	return true
}
...
// pumpMoves рассылает сигнал по заметкам, где ветки встали на место.
//
// Спрашивает ТОЛЬКО про заметки, у которых есть слушатель, и это не экономия
// ради экономии: индекс переездов ведёт с note_id (см. movedNotesQuery), а
// «покажи все переезды» без него — перебор таблицы на 10,7 млн строк. Список
// открытых тредов у хаба и так есть, и он же есть точное определение того, кому
// этот сигнал вообще может пригодиться: никто не смотрит — незачем и спрашивать.
//
// Что именно переехало, хаб не выясняет: на проводе сигнал, а строки страница
// возьмёт своим курсором на /fresh — тем самым, который отдаёт X-Fresh-Moved.
func (h *hub) pumpMoves(ctx context.Context) {
	src, ok := h.src.(LiveMover)
	if !ok {
		return
	}
	notes, from := h.watchedNotes()
	if len(notes) == 0 {
		return
	}
	moved, err := src.MovedNotesSince(ctx, notes, from)
	if err != nil {
		h.log.Warn("живой канал: переезды", "err", err)
		return
	}
	for id, at := range moved {
		h.publish(noteTopic(id), liveMsg{Kind: "move", Note: id})
		if at.After(from) {
			from = at
		}
	}
	h.mu.Lock()
	// Курсор двигается ВПЕРЁД и только вперёд: между выборкой и этой строкой
	// мог пройти проход, поставивший его дальше.
	if from.After(h.moves) {
		h.moves = from
	}
	h.mu.Unlock()
}

// watchedNotes — за какими тредами кто-то следит прямо сейчас, и с какой
// отметки спрашивать. Обе величины лежат под одним мьютексом, поэтому берутся
// одним заходом: разными они разъехались бы ровно на переездах, случившихся
// между двумя взятиями.
func (h *hub) watchedNotes() ([]int64, time.Time) {
	h.mu.Lock()
	defer h.mu.Unlock()
	var notes []int64
	for t := range h.subs {
		id, ok := strings.CutPrefix(t, "note:")
		if !ok {
			continue
		}
