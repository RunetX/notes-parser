...
func TestConsentsPageShowsTheRealDocuments(t *testing.T) {
	...
	// ПЯТЬ, и последние три — необязательные: привязка мессенджера, личная
	// переписка (оба 11.09.2026) и рассказ о себе с фотографиями (12.09.2026).
	// Стоят они здесь по отдельному доводу: документ, который видит только
	// нажавший «Привязать», «Написать» или «Рассказать о себе», прочесть заранее
	// негде, а решать, подписывать ли, человек должен ДО нажатия. Обязательными
	// от присутствия в этом списке они не становятся — это стережёт соседний
	// тест про дверь (TestOptionalConsentIsNotAskedAtTheDoor).
	//
	// Число здесь названо вслух намеренно: список документов площадки — не та
	// вещь, которая растёт незаметно. Прибавился документ — прибавилась и
	// строка, где это признано.
	if len(docs) != 5 {
		t.Fatalf("согласий %d, а их пять", len(docs))
	}
	for _, d := range docs {
