...
func TestConsentsPageShowsTheRealDocuments(t *testing.T) {
	...
	// ЧЕТЫРЕ, и последние два — необязательные: привязка мессенджера и личная
	// переписка (оба 11.09.2026). Стоят они здесь по отдельному доводу: документ,
	// который видит только нажавший «Привязать» или «Написать», прочесть заранее
	...
	if len(docs) != 4 {
		t.Fatalf("согласий %d, а их четыре", len(docs))
	}
	for _, d := range docs {
