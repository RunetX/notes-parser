...
// что и ядро (platform.CurrentConsentDocs), с теми же реквизитами оператора. С
// 11.09.2026 их ТРИ, и одно из них при входе не показывается вовсе:
// необязательное согласие на привязку мессенджера подписывают на своём экране.
// Здесь берутся ВСЕ — страница отвечает на вопрос «какие документы у площадки
// есть», а не «что мне подсунут на входе». Копия документа, живущая своей жизнью, — это ровно
...
func (s *Server) handleConsentDocs(w http.ResponseWriter, r *http.Request) {
	...
	s.render(w, r, http.StatusOK, "doc.gohtml", docPage{
		...
		// Числа здесь нет НАМЕРЕННО: документов стало три, и всякая прописанная
		// словом цифра разойдётся с ядром на следующем же документе — тем же
		// доводом, по которому пороги частоты приезжают в справку из platform.
		// Про обязательность сказано потому, что она у них разная: два
		// подписывают при входе, третий — только тот, кто привязывает мессенджер.
		Lead: "Соглашения площадки. Два первых подписывают при входе, третье — " +
			"необязательное: его подписывает только тот, кто привязывает " +
			"мессенджер. Читать их можно и не входя: решать, соглашаться ли, " +
			"проще до того, как начал.",
		Docs: bodies,
	})
}
