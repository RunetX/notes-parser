...
// выпадающее меню.
//
// Пункт про себя ОДИН, и это решение владельца 12.09.2026: «к чему отдельно
// „Мой профиль" и „Моя страница"». Утром того дня их было два — своя страница
// показывала список тумблеров, чужая карточку, — и человек выбирал между двумя
// словами про одно и то же. Второе слово («Мой профиль») отсюда ушло, и тест
// стережёт, чтобы оно не вернулось: вкладки живут ВНУТРИ страницы.
func TestAccountMenuHasProfileAndExit(t *testing.T) {
	...
	for _, want := range []string{`<details class="acct">`, `<summary`, `href="/me"`,
		"Моя страница", `action="/logout"`, "Выход"} {
		if !strings.Contains(head, want) {
			t.Errorf("в меню участника нет %q", want)
		}
	}
	if strings.Contains(head, "Мой профиль") {
		t.Error("в меню снова два пункта про себя — владелец просил один")
	}
}
