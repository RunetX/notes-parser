...
func TestPagerFitsOnePhoneLine(t *testing.T) {
	...
	for _, want := range []string{"white-space: nowrap", ".pg.arr .wd { display: none; }", ".pg.far { display: none; }"} {
		if !strings.Contains(mobile, want) {
			t.Errorf("в мобильных стилях нет %q — постраничка снова ляжет в две строки", want)
		}
	}
	// Переносу взяться неоткуда и по устройству: постраничка на телефоне —
	// флексовый ряд, а flex-wrap ей не выдан. Заведись он — вернулся бы ровно
	// тот обломок второй строки, из-за которого правило и появилось.
	pages := cssRule(t, mobile, ".pages {")
	if !strings.Contains(pages, "display: flex") {
		t.Error("постраничка на телефоне не растянута на ширину экрана")
	}
	if strings.Contains(pages, "flex-wrap") {
		t.Error("постраничке выдан перенос — «След. »» снова уедет второй строкой")
	}
	// Кнопка постранички — под палец, как и всё нажимаемое на телефоне.
	// Промах тут дороже обычного: уехал не в ту страницу ленты, а «назад» у
	// постранички нет.
	if !strings.Contains(cssRule(t, mobile, ".pg {"), "min-height: 40px") {
		if !strings.Contains(cssRule(t, cssRule(t, cssText(t), "@media (max-width: 400px)"), ".pg {"), "min-height: 40px") {
			t.Error("кнопки постранички на телефоне меньше пальца")
		}
	}
...
func TestMobileFreesNoteTextWidth(t *testing.T) {
	...
	// Поле у колонки текста ОДНО, а не два вложенных. 10px у .main плюс 10px у
	// .wrap внутри — это 40px на обе стороны, десятая часть экрана в 389
	// пикселей, отданная подложке, которой на телефоне не видно вовсе:
	// карточка там идёт от края до края.
	if !strings.Contains(mobile, ".main .wrap { padding: 0; }") {
		t.Error("у колонки текста снова два поля одно в другом")
	}
}

// [Э] На телефоне шапка и подвал отбиты от края экрана ровно так же, как
// колонка текста: название площадки и значок участника упирались в самый край
...
//
// Числа здесь ДВА, но значение одно, и тест держит именно это: колонка текста
// уехала с 20px на 12 (26.08.2026, снятое второе поле — см. соседний тест), и
// шапка обязана была уехать вместе с ней. Разъедься они — шапка читается как
// съехавшая, а поймать это глазом на своём экране владельца некому.
func TestMobileHeaderKeepsEdgeMargin(t *testing.T) {
	mobile := cssRule(t, cssText(t), "@media (max-width: 700px)")
	if !strings.Contains(mobile, ".top .wrap, .foot .wrap { padding: 0 12px; }") {
		t.Error("на телефоне название площадки и значок участника прижаты к краю экрана")
	}
	if !strings.Contains(mobile, ".main { padding: 12px 12px 20px;") {
		t.Error("поле колонки текста разъехалось с отступом шапки")
	}
