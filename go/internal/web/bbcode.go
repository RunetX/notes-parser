...
//
// ФОРМ У ЦВЕТА ДВЕ, и старшая — через ДВОЕТОЧИЕ: «[color:red]». Замер
// 07.09.2026 по всему архиву: двоеточие — 1 196 вхождений в 640 текстах с 2005
// по 2015 год, равенство — 254 в 163 текстах и не раньше 2011-го, то есть
// знакомая нам запись пришла на смену старой и застала лишь последние три года
// разметки. Ровно тот же урок, что с тильдой у смайлов неделей раньше: набор
// знаков спрашивают у КОРПУСА, а не у списка, который уже знаешь.
var bbRe = regexp.MustCompile(`(?i)\[(/?)(b|i|u|s|color)(?:[=:](#?[a-z0-9]{1,12}))?\]`)

// bbColors — цвета, которые площадка умеет ПОКАЗАТЬ. Список закрытый: цвет
// приезжает классом, а класс должен существовать в стилях; незнакомый цвет не
// ошибка и не повод печатать тег буквально — он просто не красит (так же
// остаются некрашеными два десятка «#666666» и одиночные chocolate, seagreen,
// deeppink: инлайнового style у нас нет и не будет).
//
// Шесть первых сняты замером по выгрузкам досье 18.08.2026, ещё шесть — полным
// замером архива 07.09.2026, когда открылась старая форма «[color:red]»:
// white 41, brown 31, cyan 23, black 20, yellow 20, grey 5. Замер тот же самый,
// что назвал двоеточие, — и он же объясняет, почему их не было видно: белым и
// чёрным писали ДО 2011 года, а прежний список снимался с текстов, где старой
// формы почти нет.
//
// Белый и чёрный красятся не собой, а токенами темы (см. style.css): на сайте
// «белый» значило СЛИТЬСЯ С ФОНОМ (им прятали точки-отбивки: «[color:white]
// ........[/color]»), а «чёрный» — цвет обычного текста. Взяв их буквально, мы
// на тёмной теме показали бы спрятанное и спрятали бы показанное.
var bbColors = map[string]bool{
	"red": true, "green": true, "blue": true,
	"purple": true, "orange": true, "pink": true,
	"white": true, "black": true, "cyan": true,
	"brown": true, "yellow": true, "grey": true,
}

// bbColorNames — цвета, которые площадка ПРЕДЛАГАЕТ: по-русски и в порядке
// частоты в архиве, потому что справочник под формой набирают глазами, а «pink»
// в списке кодов ничего не говорит тому, кто пришёл написать заметку.
//
// Список короче bbColors, и это не недосмотр: показать мы умеем и то, чем
// писали в 2008-м, а предлагать белый (сливается с фоном) и чёрный (цвет
// обычного текста) значит звать человека написать невидимое. Подсказка — не
// каталог возможностей. Обратное включение обязательно: предложенный цвет
// обязан краситься, иначе подсказка обещает то, чего не будет, — стережёт
// TestMarkupHelpCoversEveryOfferedColor.
...
func markupSample(text string) template.HTML {
	...
	st := newBBState(era{markup: true}, text)
...
type bbTag struct {
	...
	line  bool   // закрывающего нет вовсе: живёт до конца СТРОКИ
}
...
type bbState struct {
	...
	// solo — по флагу на КАЖДЫЙ открывающий тег текста, в порядке появления:
	// закрывающего для него нет нигде дальше. Считается один раз на весь текст
	// (bbUnclosed), потому что строка о будущем не знает, а решение нужно
	// принять в тот момент, когда тег открывается.
	solo []bool
	nth  int // сколько открывающих уже встречено — индекс в solo
}

// newBBState — разбор одного текста. Конструктор, а не литерал, ровно ради
// solo: забыв его заполнить, мы получили бы прежнее «незакрытое красит всё до
// конца заметки» молча, и разошлись бы два способа показать один текст.
func newBBState(e era, text string) bbState {
	// Выключенная разметка и текст без скобки — самый частый случай площадки
	// (10,7 млн зеркальных строк после заката), и лишний проход по телу там
	// не нужен: solo читает только line(), и только когда разметка включена.
	if !e.markup || !strings.Contains(text, "[") {
		return bbState{era: e}
	}
	return bbState{era: e, solo: bbUnclosed(text)}
}

// bbUnclosed отвечает про каждый открывающий тег текста: закрыт он где-нибудь
// дальше или нет. Стек здесь тот же, что у close(): закрывающий гасит
// БЛИЖАЙШИЙ подходящий, а теги, открытые позже, остаются открытыми —
// перекрёстная разметка «[b][i]…[/b][/i]» в архиве обычное дело.
func bbUnclosed(text string) []bool {
	var (
		solo  []bool
		names []string
		stack []int // индексы в solo: что открыто и ещё не закрыто
	)
	for _, m := range bbRe.FindAllStringSubmatchIndex(text, -1) {
		name := strings.ToLower(text[m[4]:m[5]])
		if m[3] > m[2] { // группа «/» непустая — закрывающий
			for i := len(stack) - 1; i >= 0; i-- {
				if names[stack[i]] == name {
					solo[stack[i]] = false
		...
		solo = append(solo, true)
		names = append(names, name)
		stack = append(stack, len(solo)-1)
	}
	return solo
}
...
func (s *bbState) line(b *strings.Builder, line string) {
	...
	for _, m := range bbRe.FindAllStringSubmatchIndex(line, -1) {
		...
		// Счётчик двигается ДО потолка вложенности: solo считан по всему
		// тексту, и пропущенный push не должен сдвигать нумерацию.
		solo := s.nth < len(s.solo) && s.solo[s.nth]
		s.nth++
		s.push(b, bbTag{name: name, color: color, line: solo})
	}
	if txt := line[idx:]; txt != "" {
		s.flush(b)
		writeText(b, txt, s.era.smiles, s.era.links)
...
// endLine закрывает теги, у которых закрывающего нет вовсе: такой тег красит
// СВОЮ строку и дальше не идёт (решение владельца 07.09.2026 — так это
// выглядело на НГС).
//
// Правило узкое намеренно: тег, закрытый где-то дальше, строку переживает, и
// «[color:purple]» из заметки 60950, закрытый через три строки, красит все три.
// Стоит это (замер по архиву до заката, 487 401 текст с разметкой) ровно 194
// текстов: незакрытый [b] в 150 из них перетекал на следующую строку, [i] в 43,
// [color] в одном. Прежнее поведение — до конца ЗАМЕТКИ — красило хвост целиком.
//
// Закрывается вместе с ним и всё, что открыто ПОЗЖЕ (иначе элементы
// пересекутся), но не теряется: закрытое где-то дальше уходит в отложенные и
// вернётся на следующей строке.
func (s *bbState) endLine(b *strings.Builder) {
	first := -1
	...
	var back []bbTag
	for j := len(s.open) - 1; j >= first; j-- {
		b.WriteString(s.open[j].closeTag())
		if j > first && !s.open[j].line {
			back = append([]bbTag{s.open[j]}, back...)
		}
	}
	s.open = s.open[:first]
