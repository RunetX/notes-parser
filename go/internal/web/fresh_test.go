...
// Вопрос, приехавший ЖИВЫМ ДОБОРОМ, обязан приехать с вариантами.
//
// Это НЕ то же самое, что реакции, которые добор намеренно оставляет пустыми:
// реакцию новой реплике ещё некому было нажать, а варианты кладутся ТОЙ ЖЕ
// транзакцией, что и сама реплика, — у вопроса они есть с рождения. Поймано
// владельцем на первом боевом вечере 11.09.2026: четвёртый вопрос выпуска встал
// на открытую страницу голым текстом, а варианты появились только после
// обновления — то есть ровно то, чего живой добор и должен избежать.
//
// Проверка идёт через настоящий обработчик, а не через функцию: разметка и
// запрос к ядру порознь были верны, дефект жил в том, что одно не звало другое.
func TestFreshQuestionCarriesItsOptions(t *testing.T) {
	st := quizStore(-1)
	st.fresh = []platform.CommentView{freshComment(1, 0, 0)}
	h := openServer(t, st)

	body := do(h, guest(t, "GET", "/n/312811/fresh?after=0,0")).Body.String()
	if !strings.Contains(body, `class="quiz`) {
		t.Fatalf("у добранного вопроса нет коробки вариантов:\n%s", body)
	}
	if n := strings.Count(body, "qbtn"); n != 3 {
		t.Errorf("кнопок вариантов %d, ждали три:\n%s", n, body)
	}
	// И разгадки в куске нет по тому же правилу, что на полной странице:
	// добор не должен становиться второй дверью к правильному ответу.
	if strings.Contains(body, "261 реплика") {
		t.Error("разгадка уехала в кусок добора")
	}
}
