...
func (s *Server) editTarget(w http.ResponseWriter, r *http.Request) (platform.User, platform.NoteView, editMode, bool) {
	...
	case admin && platform.IsNative(note.ID):
		// Администраторская дверь СИЛЬНЕЕ авторского окна, и стоит она первой
		// именно поэтому (10.09.2026, жалоба владельца «не могу добавить
		// картинку к своей же нативной заметке»). Прежде первым проверялось
		// окно, и в свои десять минут администратор проваливался в editOwn —
		// то есть на форме своей свежей заметки поля файла не было вовсе, а на
		// чужой зеркальной было. Замер по логу боевого хоста: заметку
		// 100000000042 открывали на правку дважды через семь минут после
		// публикации, оба раза GET и ни одного POST — смотреть было не на что.
		//
		// Порядок этот к тому же честнее по существу: своё окно у автора одно и
		// тратится один раз, а администратор правит и позже и не тратя его, —
		// значит окно ему просто не нужно, а вот запись в журнал модерации
		// нужна, и с этой дверью она есть.
