...
(function () {
  ...
  var insert = function (html, moved) {
    ...
    var tail = mail && atEnd(list) && !selected();
    var box = document.createElement('template');
    box.innerHTML = html;
    ...
    // Дочитанный до конца разговор остаётся дочитанным. Письмо встаёт в конец
    // блока, который прокручивается внутри себя, — без этой строки живой добор
    ...
    if (tail && fresh.length) list.scrollTop = list.scrollHeight;
    if (first) {
...
// Отсюда эти три строки: без них переписка каждый раз открывалась бы началом
