...
// КОНЕЦ РАЗГОВОРА — ЭТО РАМКА ЛИБО СТРАНИЦА (эпик L).
//
// Список писем прокручивается внутри себя, и рамку эту рисует CSS — а на
// телефоне не рисует вовсе (style.css, ветка 700px: там она поле ответа всё
// равно не удержит, зато прокруток под пальцем стало бы две). Значит «поставить
// разговор на конец» означает РАЗНОЕ в двух случаях, и спрашивать надо не ширину
// экрана — второе место, где написан порог, разъедется с первым молча, — а саму
// рамку: прокручивается она сейчас или нет.
//
// Спрашивают это ДВОЕ — открытие страницы и живой добор, — и написанное у них
// порознь разошлось бы так же молча. Поэтому функции, а не строчка на месте.
function talkScroller(list) {
  return list && list.scrollHeight > list.clientHeight + 1 ? list : null;
}

// Ставит разговор на конец, и работы тут ДВЕ, а не одна на выбор.
//
// Первая — сама рамка, её собственной прокруткой. Вторая — СТРАНИЦА, и подводят
// её к ФОРМЕ ОТВЕТА, а не к последнему письму: форма стоит под разговором, и
// подведя к ней, мы показываем и её, и хвост разговора над ней. Формы нет у
// закрытой переписки (там причина словами) — тогда концом служит письмо.
//
// Вторая нужна не только там, где нет рамки. Замер 12.09.2026 (окно 1264×765,
// настоящий Chrome): рамка в 60vh — 457 точек, над ней карточка собеседника и
// липкая шапка — 232, под ней форма — 239, итого 928 при экране в 765. То есть
// «поле ответа остаётся на виду» рамка не исполняла и на десктопе, просто
// промахивалась мягче: кнопка отправки лежала на 140 точек ниже края. Подвести
// страницу дешевле, чем считать высоту рамки формулой из чужих высот (шапка,
// постраничка, число кнопок у собеседника) — такая формула врёт молча при
// первой же правке любой из них.
//
// page=false — «страницу не трогать»: так зовёт живой добор, когда человек
// набирает ответ (правило handsBusy).
function talkToEnd(list, page) {
  var box = talkScroller(list);
  if (box) box.scrollTop = box.scrollHeight;
  if (!page) return;
  var to = document.querySelector('.wbox') || (list && list.lastElementChild);
  if (!to) return;
  // Уже на виду — не двигаем ничего: короткий разговор помещается в экран
  // целиком, и «прокрутка к форме» увела бы с глаз карточку собеседника ради
  // того, что и так видно. Живого добора это касается ровно так же: письмо
  // подвинуло форму вниз, но пока она в кадре, страница стоит.
  var h = window.innerHeight || document.documentElement.clientHeight;
  var r = to.getBoundingClientRect();
  if (r.top >= 0 && r.bottom <= h) return;
  try { to.scrollIntoView({ block: 'end' }); } catch (e) { to.scrollIntoView(false); }
}
...
(function () {
  ...
  // Ключ порядка СЕСТЁР — ВРЕМЯ, затем номер. Тем же ключом идёт страница
  // (platform.OrderSiblingsByTime), и разойтись им нельзя: место, куда добор
  // кладёт строку, обязано совпасть с местом, куда её положит перезагрузка.
  //
  // Одного номера здесь мало, хотя путь в базе собран именно из номеров: свои и
  // зеркальные реплики лежат в разных полосах, и нативный номер больше любого
  // ngs'ного, — то есть ответ, написанный на НГС минутой позже, по номеру встал
  // бы ВЫШЕ реплики соседа. Номер остаётся разрывом ничьей: у зеркальных реплик
  // время с точностью до секунды.
  var when = function (el) {
    return el ? parseInt(el.getAttribute('data-time'), 10) || 0 : 0;
  };

  // Стоит ли строка a ПОЗЖЕ строки b среди сестёр.
  var laterThan = function (a, b) {
    var wa = when(a), wb = when(b);
    return wa !== wb ? wa > wb : num(a) > num(b);
  };

  // Куда встанет новая реплика в ДЕРЕВЕ. Место то же, что дало бы обновление.
  //
  ...
  var placeInTree = function (li) {
    ...
    while (at) {
      ...
      if (at.classList.contains('c')) {
        ...
        if (d === depth + 1 && laterThan(at, li)) { break; }  // нашлась младшая сестра
      }
  ...
  // ВЫДЕЛЕНИЕ — половина правила «страница не прыгает из-под рук», и нужна она
  // порознь: набор ответа прокрутке РАЗГОВОРА не помеха (каретка в поле, а не в
  // письмах), а выделенный внутри него текст — помеха, он уехал бы из вида
  // вместе с прокруткой. Поэтому handsBusy собран из этой проверки, а не она из
  // него.
  ...
  // «Стои́т в конце» — с допуском: дробные пиксели при масштабе и сдвиг подсветки
  // дают разницу в единицы точек, а точное равенство означало бы, что признак не
  // срабатывает почти никогда. Блока без прокрутки это тоже касается: короткий
  // разговор виден целиком, то есть человек стои́т в его конце по определению.
  var atEnd = function (el) {
    if (!el) return false;
    if (el.scrollHeight <= el.clientHeight) return true;
    return el.scrollHeight - el.scrollTop - el.clientHeight < 40;
  };

  // «Дочитан ли разговор» — вопрос к той поверхности, которая на этой странице
  // прокручивается (talkScroller). У СТРАНИЦЫ спрашивать про её нижний край
  // нельзя: под формой ответа лежат пояснения и подвал, до них не доходит никто,
  // и признак не срабатывал бы никогда. Спрашиваем про последнее письмо: видно
  // его — значит человек стои́т в конце разговора.
  var talkEnded = function () {
    var box = talkScroller(list);
    if (box) return atEnd(box);
    var last = list && list.lastElementChild;
    if (!last) return true;
    var h = window.innerHeight || document.documentElement.clientHeight;
    return last.getBoundingClientRect().bottom <= h + 40;
  };

  var insert = function (html, moved) {
    if (!html) return null;
    // Мерить надо ДО вставки: после неё у блока другая высота, и «был ли человек
    // в конце разговора» ответа уже не имеет.
    var tail = mail && talkEnded() && !selected();
    var box = document.createElement('template');
    box.innerHTML = html;
    ...
    // Дочитанный до конца разговор остаётся дочитанным: без этого живой добор
    // стал бы невидимым ровно у того, кто сидит в переписке, — письмо легло бы
    // ниже нижнего края. Промотавшего вверх не дёргаем, он читает старое, а
    // новое письмо подсвечено и никуда не денется.
    // РАМКУ двигаем и под руками: каретка в поле, а письма под ней не
    // шевелятся. СТРАНИЦУ — только если руки свободны: там едет и сама форма
    // ответа, то есть буквы уезжают из-под пальца (правило handsBusy).
    if (tail && fresh.length) talkToEnd(list, !handsBusy());
    if (first) {
...
// РАЗГОВОР ОТКРЫВАЕТСЯ ПОСЛЕДНИМ ПИСЬМОМ (эпик L).
//
// Блок писем прокручивается внутри себя (.lts.talk в style.css), и рамку эту
// рисует CSS — без скрипта она работает как работала. А поставить блок на
// последнюю строку силами CSS нечем: свойства «открой меня с конца» у браузеров
// пока нет, и трюк с column-reverse здесь не годится — он переворачивает саму
// разметку, то есть порядок, в котором разговор читают диктор и буфер обмена.
// Отсюда эти строки: без них переписка каждый раз открывалась бы началом
// страницы, то есть письмами полугодовой давности.
//
// На телефоне рамки нет вовсе, и обещание исполняется прокруткой САМОЙ страницы
// (talkToEnd): там это не украшение, а единственное, что ставит форму ответа
// на экран, — с неё и началась жалоба 12.09.2026.
//
// Живёт это отдельной частью, а не в живом доборе: тот начинается с проверки
// колокольчика и EventSource, а прокрутка нужна и там, где потока нет вовсе.
//
// Ставится ОДИН раз, хотя defer работает до картинок и до подмены шрифта:
// высота письма после разбора страницы не меняется — у смайла размер стои́т
// атрибутами (он прочитан из самого GIF), роликов в письме не бывает вовсе
// (`vid` зовут заметка и реплика, но не письмо), а шрифт темы до текста письма
// не доходит — им набраны заголовки да служебные строки. Заведись здесь
// картинка без размеров, понадобился бы второй проход по load, и вот ЕГО
// пришлось бы отличать от руки человека.
//
// АНКОР СИЛЬНЕЕ. Пришедший по ссылке на письмо шёл читать ИМЕННО ЕГО, и браузер
// уже привёл его куда надо — перебить это прокруткой в конец значило бы
// отменить чужое указание (то же правило, по которому `:target` разворачивает
// простыню и открывает просмотрщик картинок).
(function () {
  'use strict';

  if (location.hash) return;
  var talk = document.querySelector('.lts.talk');
  if (talk) talkToEnd(talk, true);
})();
