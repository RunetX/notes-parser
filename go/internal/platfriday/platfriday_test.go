...
// Обращение в НАЧАЛЕ реплики — стоп. Поймано первым черновиком на боевом архиве
// 11.09.2026: «Звёздная, ага..я тож..», «Антон, чтоб заметки в анкете не висели»
// и «Алюминиевый сквозняк, нет, конечно» вышли в вопросы живыми именами — при
// том что вступительная заметка обещает, что имён рубрика не называет.
//
// nickRe этого не видел по устройству: он ловит заглавную букву ПОСЛЕ слова, а у
// обращения место первое. Цена правила названа в самом коде — «Да, конечно»
// отвергается вместе с никами.
func TestLeadingAddressIsRejected(t *testing.T) {
	for _, tc := range []struct {
		text string
		want bool
	}{
		{"Звёздная, ага..я тож..смысл с титек в личке?", true},
		{"Антон, чтоб заметки в анкете не висели)))", true},
		{"Алюминиевый сквозняк, нет, конечно))) нам самим мало)))", true},
		{"Эй,вы!? Чо эфир занимаете?", true},
		{"крок, ну ты даёшь", true},
		// Три слова до запятой — это уже речь, а не обращение.
		{"Вы не представляете, как я люблю холодок", false},
		{"А почему анонимно? Теперь не знаем кому слать)))", false},
		{"Зачем просыпаться в 6 часов, когда тебе не на работу", false},
	} {
		if got := looksAddressed(tc.text); got != tc.want {
			t.Errorf("%q → %v, ждали %v", tc.text, got, tc.want)
		}
	}
}
