...
// Пол уходит в промпт СЛОВОМ, а не выводится из ника. Первый живой прогон
// 28.08.2026 показал, чем это кончается: событие пишется от первого лица, и
// «собрался разобрать балкон» у Татьяны выдаёт подмену одним глаголом.
func TestInnerPromptNamesGender(t *testing.T) {
	got := innerPrompt(&Card{Persona: Bio{Nick: "Полынь-Трава", Gender: "female", Age: 46}},
		"", time.Date(2026, 6, 4, 0, 0, 0, 0, time.UTC))
	if !strings.Contains(got, "женщина") {
		t.Fatalf("пол в промпте не назван:\n%s", got)
	}
}

// Время года проверяется КОДОМ, а не просьбой: дата в промпте стояла и в первом
// живом прогоне 28.08.2026 — модель её прочитала и всё равно написала Полынь-
// Траве «скидывал снег с машины, дворники примёрзли» вторым июня.
func TestInnerSeasonMiss(t *testing.T) {
	june := time.Date(2026, 6, 2, 0, 0, 0, 0, time.UTC)
	if innerSeasonMiss("скидывал снег с машины, дворники примёрзли", june) == "" {
		t.Fatal("снег вторым июня прошёл проверку")
	}
	if got := innerSeasonMiss("сходил в гараж за канистрой", june); got != "" {
		t.Fatalf("обычное июньское событие отвергнуто: %s", got)
	}
	jan := time.Date(2026, 1, 15, 0, 0, 0, 0, time.UTC)
	if innerSeasonMiss("весь день загорали на пляже", jan) == "" {
		t.Fatal("пляж в январе прошёл проверку")
	}
	// Весна и осень не проверяются вовсе: снег в апреле под Новосибирском —
	// правда, и запрет тут чинил бы достоверность выдумкой.
	apr := time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)
	if got := innerSeasonMiss("с утра лёг снег, чистил дорожку", apr); got != "" {
		t.Fatalf("апрельский снег отвергнут: %s", got)
	}
}

// Корни сверяются с НАЧАЛОМ СЛОВА. Правило оплачено живым прогоном 28.08.2026:
// подстрочное «ник » выбрасывало понедельник, субботник, пикник, дневник,
// праздник и сантехника — и увидеть это удалось только по нулю событий за два
// треда, потому что все проверки до того были написаны на площадочных примерах,
// где подстрока и корень совпадают.
func TestInnerRejectDoesNotEatOrdinaryWords(t *testing.T) {
	fine := []string{
		"В понедельник поехал за картошкой",
		"Сходил на субботник, потом пикник у соседей",
		"Внук принёс дневник с тройкой",
		"Купила ленту для шляпы",
		"Сломалась ветка яблони, спилил",
		"Заходил сантехник, менял кран",
		"В праздник сидели дома",
	}
	for _, s := range fine {
		if why := innerReject(s); why != "" {
			t.Errorf("обычная фраза отвергнута: %q (%s)", s, why)
		}
	}
	bad := []string{
		"Прочитал заметку про дачу",
		"Ответил в треде и пошёл спать",
		"Весь вечер сидел в интернете",
	}
	for _, s := range bad {
		if innerReject(s) == "" {
			t.Errorf("площадочная фраза прошла: %q", s)
		}
	}
}
