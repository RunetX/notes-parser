...
type Card struct {
	...
	// RoleLead — можно ли этому жителю открывать реплику ролевой рамкой («как
	// пожарный скажу»). АВТОРСКОЕ РАЗРЕШЕНИЕ владельца, а не замер: по замеру
	// (10,8 млн реплик архива) у живых такой зачин стоит в 0,0004 % реплик, то
	// есть его нет ни у кого. Разрешение поимённое и на пару человек: у одного
	// оборот читается манерой, у тридцати — машиной, а флаг, в отличие от доли,
	// принадлежит ЧЕЛОВЕКУ, а не реплике.
	RoleLead bool `json:"role_lead,omitempty"`
	// Noise — житель-ШУМ: пишет коротко и мимо, ветку не читает, реагирует на
	// заметку. Не украшение: в живом треде фон из отмашек и «плюсую» и есть то,
	// на чём видно остальное, а у нас каждая реплика была работой мысли.
	//
	// Флаг меняет ТРИ вещи разом (окно, длину и строгость евалов), и потому он
	// флаг, а не набор настроек: «шум» — это порода жителя, и собирать её из
	// трёх независимых тумблеров значило бы завести четыре породы, из которых
	// три бессмысленны.
