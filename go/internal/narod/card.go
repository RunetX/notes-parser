...
type Register struct {
	...
	// ЧЕМ НАПОЛНЕНА РЕПЛИКА — замер `internal/speech`, заведённый 30.08.2026 по
	// жалобе на первую живую песочницу («реплики какие-то абстрактные»). До него
	// карточка описывала, КАК человек пишет, и молчала о том, ЧЕМ он говорит.
	//
	// Первый же замер показал, что дефект не там, где казалось. Свой случай наши
	// поминают даже чаще доноров (15,0 % против 2,8–12,7 %), а расходятся два
	// других числа: ЧИСЕЛ у нас 1,0 % против 5,8–17,7 % у доноров и 11,2 % по
	// корпусу, ОБОБЩЕНИЙ 34,3 % против 11,6–18,7 %. То есть абстрактность здесь
	// — это не «нечего рассказать», а «сказано про всех и без единой цифры».
	//
	// Меры ЛИЧНЫЕ, как разговорчивость: у доноров размах троекратный, и общая
	// доля на всех сделала бы тридцать жителей одинаковыми ровно там, где живые
	// разные.
	DigitRate   float64 `json:"digit_rate"`   // доля реплик с числом
	GeneralRate float64 `json:"general_rate"` // доля реплик с обобщением
	// AdviceRate замерена, но РЫЧАГА у неё пока нет, и это названо нарочно:
	// разрыв тут меньше прочих (10,3 % у нас против 1,3–7,0 % у доноров), а две
	// правки за заход — уже предел, при котором видно, которая из них подействовала.
	AdviceRate float64 `json:"advice_rate"` // доля реплик с поучением
}
