...
var worldMigrations = []string{
	...
	worldMigrationV2, // v2 — выпавший ход в журнале генерации
}

// worldMigrationV2 — ХОД, которым житель говорил, в журнале генерации.
//
// Заведено 11.09.2026 по разбору боевого треда 100000000048 (148 генераций).
// Ход выпадает жребием зовущего (move.go) и задаёт форму реплики сильнее всего
// прочего, а в журнале его не было вовсе: на вопрос «почему реплики
// однообразны» отвечать было нечем, кроме догадки. Замер содержания говорил
// «свой случай» в 27,7 % реплик при доле хода в 15 %, и разойтись эти числа
// могли по двум разным причинам — жребий не доехал до боя либо истории текут
// через остальные шесть ходов, — а различает их только запись о том, что
// выпало на самом деле.
//
// Схема v1 при этом не правится: worldschema.sql накатывается на пустую базу
// первой миграцией, и колонка, добавленная туда же, второй раз добавиться бы
// не смогла (тот же приём, что в store.migrateList).
const worldMigrationV2 = `
ALTER TABLE gen_runs ADD COLUMN move TEXT NOT NULL DEFAULT '';
`
