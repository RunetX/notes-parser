...
// writeTriggers — раздел про СЛОВА, а не про кубик.
//
// У темы два числа, и они про разное: ВЕС («любит или морщится») пишет владелец
// и читает модель, ПЕРЕКОС (Lift) — замер, и читает его один кубик. Тема,
// приехавшая от донора ради перекоса, веса не имеет вовсе, и раздела ей не
// положено: заголовок без единой строки под ним модель прочтёт как пропуск, а
// перечислить донорские интересы значило бы написать жителю биографию за
// владельца. Пустой заголовок и вышел на первом же пересъёме 31.08.2026, когда
// перекос впервые доехал до карточки.
