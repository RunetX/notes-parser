...
type Config struct {
	...
	// StampRate — доля реплик, где РАЗРЕШЁН штамп машины (складно, вслух,
	// ровно, будто, верно, легко, нечего, потому — список в internal/speech).
	// ЗАМЕР 01.09.2026: у живых реплик той же длины, что наши (15–35 слов),
	// хотя бы одно такое слово стоит в 2,94 % — против 23,0 % у нас на 849
	// боевых репликах. Доля КОРПУСНАЯ: в карточке личного замера нет, и
	// сказано это здесь, а не подразумевается.
	//
	// Полоса длины на число ВЛИЯЕТ, и это названо: свод (narod stamps) выводит
	// её из наших же реплик как десятую и девяностую доли, получает 5–49 слов
	// и человеческую долю 1,9 %. Взято БОЛЬШЕЕ из двух: щедрость здесь дешевле
...
func (c Config) Validate() error {
	...
	if c.StampRate < 0 || c.StampRate > 1 {
		return fmt.Errorf("доля штампов %v: бывает от нуля до единицы", c.StampRate)
	}
	if c.LatencyScale <= 0 {
...
func (s *Service) compose(ctx context.Context, p Player, pl Plan,
	...
	// ШТАМП МАШИНЫ. Доля общая, а не личная: это не почерк жителя, а почерк
	// модели, одинаковый у всех шестидесяти.
	stamp := rng.Float64() < s.cfg.StampRate
	point.Stamp = &stamp
	// ПРИЁМ ШУТКИ выбирается ЗДЕСЬ, а не в промпте, и с оглядкой на уже
...
// pickPunch — приём шутки на эту реплику. Пустая строка невозможна: набор
// непуст, и «шутить нечем» здесь не состояние, а поломка.
//
// asks — выпал ли жребий вопроса. Приёмы, которые СОСТОЯТ из вопроса,
// берутся только тогда, когда вопрос разрешён; обратного отсева нет —
// вопрос можно задать и не встречным.
func pickPunch(rng *rand.Rand, asks bool) string {
	allowed := make([]string, 0, len(punchForms))
	for _, f := range punchForms {
		if !asks && f.Name == "встречный вопрос" {
			continue
		}
