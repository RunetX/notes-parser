...
// процентов, а общее слово стояло как стояло. К тому же ThreadWindow — одно
// число на бой и калибровку, и трогать его здесь нельзя: голос мерился бы при
// одном объёме контекста, а писался при другом. Поэтому здесь режется не ОКНО,
// а сам повтор: слово, уже сказанное тремя разными жителями и отсутствующее в
