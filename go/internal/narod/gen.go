...
type WritePoint struct {
	...
	Stamp *bool
	// Move — ЧТО житель делает этой репликой: шутит, спрашивает автора, ворчит,
	// отписывается двумя словами (move.go). Выпадает жребием зовущего — как
	// длина и эмодзи, и по тому же доводу: набор ходов живёт МЕЖДУ репликами.
	Move Move
	// Canon — слова, которые в треде уже повторили трое разных жителей и
	// которых не было в заметке (canon.go). В промпт идут запретом, в евал —
	// проверкой: выдуманный факт вправе подхватить двое-трое, а не весь тред.
	Canon []string
...
func checkDraft(text string, p WritePoint) string {
	...
	// РОЛЕВОЙ ЗАЧИН — «как пожарный скажу». Стои́т среди ЖЁСТКИХ проверок, а не
	// среди стилевых, и потому спрашивается даже у жителя-шума: это не мерка
	// письма, а примета машины, замеренная по корпусу (0,0004 % у живых против
	// большинства реплик в первом же треде с панчем). Разрешение — поимённое,
	// флагом карточки.
	if !p.Card.RoleLead {
		if s := speech.RoleLead(text); s != "" {
			return "ролевой зачин «" + s + "» — так не начинают, скажи это без представления по должности"
		}
	}
	// ЧУЖОЕ РЕМЕСЛО. В прогоне 01.09.2026 Тонкая рябина побывала в одном треде
	// ветеринаром и сантехником, Фельдшер Оля — кладовщиком, Прораб Витёк —
	// отделочником и ветеринаром: образец приёма оказался сильнее карточки.
	// Ремесло — жёсткий факт о человеке, как имя и город, и врать о нём житель
	// не вправе; проверка поэтому стои́т рядом с приметами машины, а не среди
	// мерок письма.
	if claim := speech.SelfRole(text); claim != "" && !tradeMatches(claim, p.Card) {
		return "ты не " + claim + ": твоё дело — " + p.Card.Persona.Job + ", чужим ремеслом не называйся"
	}
	// КАНОН треда: слово, придуманное по ходу и повторённое уже тремя. Проверка
	// не стилевая — она про то, что житель ЗНАЕТ, — и потому спрашивается у всех.
	if w := canonHit(text, p.Canon); w != "" {
		return "слово «" + w + "» в этом разговоре уже повторяют за другими, а в заметке его не было — скажи своё"
	}
	// ЖИТЕЛЬ-ШУМ дальше не судится. Он и заведён затем, чтобы в треде были
	// реплики, которые никто не редактировал: «плюсую» без эмодзи, без числа и
	// с обобщением — это не брак, а фон живого разговора. Всё, что выше,
	// остаётся: примета машины, мат и чужое ремесло не перестают быть собой
	// оттого, что реплика пустая.
	if p.Card.Noise {
		return ""
	}
...
// tradeMatches — совпадает ли названное ремесло с карточкой.
//
// Сверка по корню в пять букв (speech.TradeRoot): «сантехник» сходится с
// «сантехником» из биографии и расходится со «сварщиком». Ищется он не только в
// Job, но и в фактах и в нике — «Механик Сева» назван механиком прямо в имени, а
// у иного ремесло стои́т фактом («держит пасеку»).
func tradeMatches(claim string, c *Card) bool {
	if c == nil {
		return true
	}
	root := speech.TradeRoot(claim)
	hay := c.Persona.Job + " " + c.Persona.Nick + " " + strings.Join(c.Persona.Facts, " ")
	for _, w := range sitetext.Words(hay) {
		if speech.TradeRoot(w) == root {
			return true
		}
	}
	return false
