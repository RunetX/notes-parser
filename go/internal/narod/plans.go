...
// PendingOf — сколько намерений жителя ещё не исполнено, по ВСЕМ тредам.
//
// Нужен потолкам, и вот почему. Потолок считает реплики в ЖУРНАЛЕ, то есть уже
// сказанное, а между монеткой и репликой лежит замеренная задержка в минуты. В
// эти минуты житель для потолка «молчит», хотя на самом деле уже пообещал
// сказать: в людном треде он успевает набрать десяток намерений, из которых
// шесть станут репликами, а остальные умрут на исполнении — каждое потратив
// свою монетку, которая бросается РОВНО ОДИН РАЗ на событие.
//
// Пара с SaidSince и есть «сколько он уже наобещал», и спрашивается она раз на
// проход смотра, а не на точку решения.
func (w *World) PendingOf(ctx context.Context, actorID string) (int, error) {
	var n int
	err := w.db.QueryRowContext(ctx, `
		SELECT count(*) FROM plans WHERE actor_id = ? AND state = ?`,
		actorID, PlanQueued).Scan(&n)
	return n, err
}
