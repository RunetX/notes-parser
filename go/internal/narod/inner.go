...
func askInner(ctx context.Context, gen JSONGenerator, card *Card, w *World,
	...
	if innerReject(text) != "" || innerSeasonMiss(text, day) != "" {
...
// модель написала не про этот месяц.
//
// Списки короткие и закрытые — как innerForbidden и platform.AutoHideable: они
// про ПРАВО машины записать событие, а не про вкус. Оплачены первым живым
// прогоном 28.08.2026: у Полынь-Травы 23 мая «замок прихватило морозом», а
// 2 июня «скидывал снег с машины, дворники примёрзли». Дата в промпте стояла —
// модель её прочитала и всё равно написала зиму; значит проверка обязана быть
// не просьбой.
//
// Полгода посередине (весна и осень) не проверяются вовсе: в Новосибирске снег
// в апреле и в октябре — правда, и запретить его там значило бы чинить
// достоверность выдумкой.
var (
	innerWinter = []string{"снег", "мороз", "примёрз", "примерз", "сугроб",
		"гололёд", "гололед", "метел", "лыж", "ёлк", "елк"}
	innerSummer = []string{"жар", "купал", "загора", "пляж", "ягод", "грядк",
		"сенокос", "кондиционер"}
)
...
// seasonRu — время года словом. Стоит в промпте рядом с датой, потому что дату
// модель читает и всё равно пишет зиму в июне: месяц она понимает как число, а
// сезон — как обстановку.
func seasonRu(day time.Time) string {
	switch day.Month() {
	case time.December, time.January, time.February:
		return "зиму"
...
2. Обыденное сильнее яркого: ни катастроф, ни озарений.
3. Жизнь НЕ СОСТОИТ из очередей и учреждений. Поликлиника, аптека, талон, химчистка, рынок, «стоял в очереди» — это то, что приходит в голову первым, и потому у десяти разных людей выходит один и тот же день. Бери из другого: работа и то, что там случилось; дом и вещи в нём; родня и соседи; еда; дорога; погода; животные; давно отложенное дело; чужая просьба; то, что человек делает ради удовольствия.
4. Про интернет, сайты, заметки и переписку не пиши ВООБЩЕ. Это про жизнь снаружи.
5. Никаких выводов, морали и настроения: только что произошло. Как человек к этому отнёсся — не твоё дело.
6. Не повторяй того, что уже было: если у него на этой неделе ломался кран, пусть в этот раз сломается что-нибудь другое или не сломается ничего. Одно и то же учреждение дважды за месяц — это повтор.
7. Событие обязано подходить именно этому человеку — его полу, возрасту, городу, работе и семье. Пол задан явно; глаголы прошедшего времени от первого лица обязаны ему отвечать.
8. И времени года: снег, мороз и примёрзшие дворники в июне — это видно сразу.`
...
func innerPrompt(card *Card, past string, day time.Time) string {
	...
	// Пол называется ПЕРВЫМ и словом, а не выводится из ника. Первый живой прогон
	// 28.08.2026: у Полынь-Травы (Татьяна) вышло «собрался разобрать балкон», у
	// Мурены (Марина) — «поехал, стоял», а у Птички всё сошлось. То есть модель
	// угадывала пол по нику и половину угадывала неверно — а событие пишется от
	// первого лица, и один глагол прошедшего времени выдаёт подмену целиком.
	switch p.Gender {
	case "male":
		b.WriteString(", мужчина")
	case "female":
		b.WriteString(", женщина")
	}
	if p.Age > 0 {
	...
	fmt.Fprintf(&b, "\n=== ЗАДАНИЕ ===\nЧто с ним случилось %s, в %s (%s)?\n",
		day.Format("2 января"), weekdayRu(day.Weekday()), seasonRu(day))
	return b.String()
}
