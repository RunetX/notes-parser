...
type Sink struct {
	...
	log   *slog.Logger
	// nudge — кому сказать, что в заметке появилась реплика с угаданным ребром
	// (ReplyScanner.Nudge). Замыкание, а не сам обходчик: приёмнику довольно
	// уметь позвать, а собирает их вместе демон — обходчика может не быть
	// вовсе, если клиенты сайта не поднялись.
	nudge func(noteID int64)
}

// SetTreeNudge подключает обходчик дерева. Необязателен: без него догадка об
// адресате доживёт до общей очереди свежих тредов, как жила до 27.08.2026.
func (s *Sink) SetTreeNudge(f func(noteID int64)) { s.nudge = f }
...
func (s *Sink) PostComment(ctx context.Context, n store.Note, _, _ string,
	...
	// Ребро мы только что УГАДАЛИ, и это известно ровно здесь. Обходчику
	// говорят про каждую принятую реплику, а не только про ту, у которой нашлось
	// обращение: ноль (реплика встала корнем ветки) — такая же догадка, просто
	// от нехватки признаков, и мобильное дерево поправляет её тем же проходом.
	if s.nudge != nil {
		s.nudge(noteID)
	}
