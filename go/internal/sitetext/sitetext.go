...
func SameStart(ours, theirs string) bool {
	ours, theirs = dropSmiles(ours), dropSmiles(theirs)
...
// СМАЙЛ — НЕ СЛОВО, и сверять его как слово нельзя (боевой случай 04.09.2026,
// заметка 313171). Мы отправили «…увеличивать вдвое :::joy::: :::joy:::
// :::joy:::», а со страницы вернулось «…вдвое ::😂:: ::😂:: ::😂::»: смайл НГС
// живёт в тексте КОДОМ, а показывается картинкой, и разбор отдаёт на её месте
// эмодзи. Из отпечатка эмодзи выпадает сам (не буква и не цифра), а наш `joy` —
// это три полноценных слова, и списки разъезжались на три токена из
// шестнадцати, то есть ниже девяти десятых.
//
// Дефект бил в ОБА рубежа сразу, потому что оба сверяют текст этой же функцией:
// findPosted не узнал свою реплику на странице и не записал её номер, а следом
// ClaimNGSEcho не узнал её же в возвращённой зеркалом копии. Дубль вышел в тред
// и в оба канала — тот самый случай, ради которого рубежей заведено два.
//
// Убираем СИММЕТРИЧНО и с обеих сторон: код смайла и живой эмодзи. Заменять их
// общим словом-маркером нельзя — это сломало бы соседнюю починку (01.09.2026,
// `:D` → 😃): у нас там остаётся огрызок в один знак, который выпадает как
// короткий токен, а маркер на месте эмодзи добавил бы сайтовой стороне лишнее
// слово, которого у нас нет.
var reSmileCode = regexp.MustCompile(`:::[A-Za-z0-9_]{1,32}:::`)

// dropSmiles — текст без смайлов.
//
// Реплика, в которой КРОМЕ смайлов ничего нет, после этого пуста и не
// опознаётся вовсе — так было и до правки, и так и надо: сверять её было бы
// нечем, кроме числа значков, а по числу своей оказалась бы и та реплика,
// которую человек написал на сайте РУКАМИ, — и зеркало её потеряло бы. Размен
// назван в TestEmptyFingerprintMatchesNothing и не пересматривается.
func dropSmiles(s string) string {
	s = reSmileCode.ReplaceAllString(s, " ")
	var b strings.Builder
	b.Grow(len(s))
	for _, r := range s {
		switch {
		case r == 0xFE0F, r == 0x200D, r >= 0x1F3FB && r <= 0x1F3FF:
			// Модификатор — часть соседнего значка, а не второй значок рядом
			// (та же мерка, что у CountEmoji).
		case r >= 0x1F000 && r <= 0x1FAFF,
			r >= 0x2600 && r <= 0x27BF,
			r >= 0x2B00 && r <= 0x2BFF,
			r == 0x2764, r == 0x203C, r == 0x2049:
