...
type Platform struct {
	...
	Mail Mail `json:"mail"`
	// MembersOnly — читать площадку могут только вошедшие (решение владельца
	// 12.09.2026). Гостю остаётся форма входа, справка и бумаги: правила,
	// которые видно только изнутри, — не правила, а сюрприз, а политику
	// обработки закон велит открыть ЛЮБОМУ (ч. 2 ст. 18.1).
	//
	// Настройка, а не константа, и это не выдуманная гибкость: чтение здесь
	// уже открывали (18.08.2026) и закрывали обратно, то есть значения у
	// признака два настоящих. Живёт он в config.json МОРДЫ на хосте, как
	// mail.enabled, и env у него нет по той же причине: это решение о площадке,
	// а не разница между стендом и боем.
	//
	// Умолчание — ОТКРЫТО: забытая секция не должна запирать площадку молча, а
	// закрытая площадка есть решение, которое принимают вслух.
...
type Contacts struct {
	...
	// OwnerMAX — ЛИЧНЫЙ адрес владельца в MAX, а не группа и не бот: место, где
	// отвечает человек. Заведено 12.09.2026 по просьбе владельца, и повод у него
	// свой: с закрытием чтения (members_only) страница владельца открывается
	// только после входа, а чаще всего написать хотят как раз те, кто войти НЕ
	// смог, — потерявший куку, потерявший анкету НГС, не дождавшийся модерации
	// поля «о себе». Для них ссылка на /u/<id> это тупик, а группа — комната с
	// посторонними.
	OwnerMAX string `json:"owner_max,omitempty"`
