...
// Подписанта выпуска здесь БОЛЬШЕ НЕТ (05.09.2026). Прежний
// `author_profile_id` называл живого человека — своего владельца, — и это
// стоило выноса недельной сводки на love.ngs.ru под его именем: галочка
// «отправлять мои записи на НГС» стои́т у АВТОРА, а не у текста. Выпуск теперь
// подписывает сама площадка своей служебной анкетой (её заводит
// `platform migrate`), настраивать тут нечего, а настройка, которой можно
// вернуть человека в подписанты, вернула бы и дефект.
...
type Platform struct {
	...
	// Support — «Поддержать площадку»: куда идти тому, кто хочет помочь деньгами.
	...
	// Mail — личная переписка (эпик L, зона 149-ФЗ).
	Mail Mail `json:"mail"`
}

// Mail — личная переписка между участниками.
//
// Настройка здесь ровно одна, и это не скупость: потолки, сроки хранения и
// размеры живут КОНСТАНТАМИ в platform/mail.go, потому что число без довода
// назавтра меняют наугад, а срок хранения писем вдобавок установлен законом —
// его «настройка» была бы настройкой соблюдения 149-ФЗ.
//
// Выключатель же нужен, и природа у него своя. Это не «фича на паузе»: пока
// площадка не уведомила Роскомнадзор и не получила ответ юриста по ч. 4
// ст. 10.1 (docs/mail-149fz.md), принимать чужие письма ей нельзя вовсе.
// Поэтому умолчание — ВЫКЛЮЧЕНО, и забытая секция ничего не включает: при
// выключенном гейте морда не заводит ни страниц, ни пункта меню, ни кнопки
// «Написать», то есть написать письмо нечем. Схема при этом накатана, сроки
// исполняются, выдача готова — обязанность хранить и обязанность выдать
// установлены одной статьёй и появляться обязаны вместе.
//
// Env у секции нет намеренно: включение переписки — решение владельца в
// известный момент, а не разница между стендом и боем.
type Mail struct {
	Enabled bool `json:"enabled"`
}
...
// Support — «Поддержать площадку»: куда идти тому, кто хочет помочь деньгами, и
// во что обходится месяц работы. Стоит рядом с Contacts, потому что это тот же
// вопрос «куда человеку идти», и живёт в тех же двух местах — на странице
// справки да в её оглавлении.
//
// Денег площадка НЕ ПРИНИМАЕТ и ничего о них не хранит: ни платёжной формы, ни
// чужого скрипта на страницах, ни строки в базе. Приём устроен целиком на
// стороне сервиса, а здесь стоит ссылка да текст вокруг неё — и это не
// осторожность, а единственная конструкция, при которой не приходится
// переиздавать согласия: «не передаёт ваши данные третьим лицам» остаётся
// правдой ровно потому, что передавать нечего. Всё, что площадка узнает о
// платеже, — это новая категория персональных данных, то есть новая редакция
// документа и переподписка ВСЕМИ.
//
// Env у секции нет намеренно: env в проекте нужен тому, что различается между
// стендом и боем либо держит секрет, а публичный адрес сбора — ни то ни другое.
type Support struct {
	// URL — адрес сбора. Пусто — темы «Поддержать площадку» нет вовсе, ровно как
	// нет темы про мессенджеры без единого адреса: страница, зовущая нажать
	// несуществующую ссылку, хуже отсутствующей.
	URL string `json:"url,omitempty"`
	// InfraRub и ModelsRub — во что обходится месяц, ДВУМЯ строками: первое — то,
	// чем площадка живёт (сервер, домен, прокси, резервные копии), второе — вызовы
	// языковых моделей её службами (утренняя заметка, недельный выпуск, реплики
	// жителей).
	//
	// Считается только то, что тратится ИМЕННО на площадку, и граница эта проведена
	// сознательно (владелец, 11.09.2026). Подписка на ассистента, которым проект
	// пишется, сюда НЕ входит: она нужна владельцу и для других дел, и повесить её
	// целиком на площадку значило бы назвать цифру, которой не соответствует
	// предмет. Сказано об этом на самой странице вслух — «как считали» внушает
	// больше доверия, чем сама сумма.
	//
	// Статей ровно две и список закрыт: дробить дальше — значит заводить перечень,
	// который меняется чаще итога. Подробной разбивке место в квартальном
	// отчёте-заметке, где она датирована самим выпуском.
	//
	// ИТОГ здесь не хранится и храниться не должен: его считает код
	// (web.Support.TotalRub), потому что третье число, заполняемое рукой, однажды
	// разойдётся с парой — и разойдётся молча.
	InfraRub  int `json:"infra_rub,omitempty"`
	ModelsRub int `json:"models_rub,omitempty"`
	// AsOf — когда считали. Числа печатаются ТОЛЬКО вместе с ним: сумма без даты
	// через полгода врёт молча, а в этой же справке до 08.09.2026 стояло «30
	// комментариев в час» при настоящих девяноста — расходятся такие числа
	// беззвучно.
	AsOf string `json:"as_of,omitempty"`
	// Payee — чьим именем подписан сбор на стороне сервиса. Называется на нашей
	// странице заранее: незнакомое имя на чужой платёжной странице пугает сильнее,
	// чем названное до перехода.
	//
	// Поле СВОЁ, а не реквизит оператора выше: тот вшит в ТЕКСТ выпущенных
	// согласий, и правка его — новая редакция документа с переподпиской всеми.
	// Пусто — про получателя не говорится вовсе.
...
func (c *Config) collectWarnings() {
	...
	if sup := c.Platform.Support; sup.InfraRub+sup.ModelsRub != 0 {
		if sup.URL == "" {
			c.Warnings = append(c.Warnings,
				"platform.support: цена месяца задана без url — темы «Поддержать площадку» нет вовсе, и числа никому не видны")
		}
		if sup.AsOf == "" {
			c.Warnings = append(c.Warnings,
				"platform.support: цена месяца задана без as_of — число без даты молча стареет, поэтому не печатается")
		}
	}
