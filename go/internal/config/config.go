...
func Load(path string) (*Config, error) {
	cfg := &Config{
		...
		// (p25 = 42 руны, p50 = 79), а не выбраны по вкусу. Нижний порог 25, а
		// не 40: у шутки удар бывает в четыре слова, и отсекать такую реплику
		// незачем. Верхний 300, а не 400: длинная шутка — это уже рассказ.
		// max_per_day = 25 при медиане 10 заметок в сутки — это runaway-guard,
		// а не троттлинг.
		Pulpit: Pulpit{
			FeedIntervalS:    20,
			FreshnessMin:     15,
			...
			GenerateTimeoutS: 90,
			MaxPerDay:        25,
			// Шутка — не пересказ: первый пришедший в голову поворот обычно и
			// есть самый плоский, поэтому размышление тут окупается. Запас по
			// времени для этого есть: первый чужой комментарий приходит через
			// медианные 164 с, а весь наш путь на low занимал 8 с. Потолок
			// генерации поднят вместе с усилием — он накрывает ВСЕ переспросы,
			// и на medium три попытки в 45 с уже не укладывались бы.
			Effort:           "medium",
			MinRunes:         25,
			MaxRunes:         300,
			MaxLines:         12,
