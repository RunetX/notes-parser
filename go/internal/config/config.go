...
func Load(path string) (*Config, error) {
	cfg := &Config{
		...
		// по две строки на повод. Удвоен и ПОЛ (200 → 400): один потолок длины
		// не поднимает — второй проход заметку режет, и первый же прогон на
		// 1800 дал 391 знак. Настоящий рычаг объёма — число поводов, оно в
		// промпте поднято с двух-трёх до трёх-четырёх.
		Morning: Morning{
			Hour:             6,
			TZ:               "Asia/Novosibirsk",
			GraceH:           3,
