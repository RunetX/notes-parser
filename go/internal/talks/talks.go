...
type Config struct {
	...
	// AskScan — спросить у человека согласия читать его переписку. Вопрос один
	// на всё: нажатая кнопка «читать и присылать сюда» — и согласие, и выбор
	// мессенджера доставки, поэтому отдельного вопроса «куда носить» больше
	// нет. alsoElsewhere — этот сайт-аккаунт залогинен ещё где-то (спрашиваем
	// во всех, и где нажмут, туда и понесём). Сообщение с кнопками умеет
	// собрать только диалоговое ядро, поэтому это замыкание из runDaemon; nil —
	// не спрашиваем, а значит и не читаем (CLI-прогоны).
