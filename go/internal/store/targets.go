...
//
// Площадка мессенджером не является (ЛС не пишет, подписчиков не уведомляет),
// но в message_targets стоит рядом с ними: там записано, где лежит каждая
// публикация, и у неё это id строки в Postgres. Отсюда и общая колонка —
// заводить вторую таблицу значило бы искать ответ на один вопрос в двух местах.
const (
...
const (
	...
	TargetNews       = "news"        // новость проекта (ref_id = метка времени публикации)
)
