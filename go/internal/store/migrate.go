...
// migrateV12SQL — опознанное эхо: записи НГС, которые ЗЕРКАЛО НЕ НЕСЁТ, потому
// что это наши же слова, ушедшие туда из Зазеркалья (пакет platngs).
//
// Зачем таблица, а не сверка по содержимому каждый раз. Опознаётся эхо ОДИН
// раз — сверкой отпечатка текста через площадку, — а спрашивают про него на
// КАЖДОМ такте: страница треда отдаёт последние тридцать реплик, и наша будет
// среди них часами. Дальше вопрос решается по id, даром и без Postgres.
//
// И второе, важнее первого: без этой памяти реплика, выпавшая из известных,
// растила бы расхождение со счётчиком треда, а на рост расхождения зеркало
// отвечает добором СТРАНИЦ (backfillGap) — до девяти лишних запросов к сайту на
// каждую нашу отправку.
//
// FK на notes нет намеренно: эхо заметки опознаётся в ЛЕНТЕ, то есть до того,
// как заметка где-либо заведена, — и заведена она не будет вовсе.
//
// Аддитивна: откат бинарника на той же БД безопасен.
const migrateV12SQL = `
CREATE TABLE ngs_echo (
    kind    TEXT NOT NULL,                     -- note|comment
    site_id TEXT NOT NULL,                     -- id записи на НГС
    note_id TEXT NOT NULL DEFAULT '',          -- у комментария — заметка; у заметки пусто
    seen_at TEXT NOT NULL,
    PRIMARY KEY (kind, site_id)
);
CREATE INDEX idx_ngs_echo_note ON ngs_echo(note_id);
`
...
var migrations = []string{
	...
	migrateV12SQL, // v12 — опознанное эхо: своё, ушедшее на НГС, зеркало не несёт
}
