...
type VoiceShape struct {
	...
	// Marks — ЧЕМ реплика наполнена: свой случай, время, числа, обобщение,
	// поучение (internal/speech). Заведено 30.08.2026: механику письма портрет
	// мерил с первого дня, а содержание — нет, и «реплики какие-то абстрактные»
	// нечем было назвать числом. Читается только рядом с портретом донора.
	Marks speech.Marks `json:"marks"`
