...
// DefaultModel — самая дешёвая текстовая модель. Для триажа по закрытому
// списку её достаточно: замер 23.08.2026 на смешанной пачке (грубость, флирт,
// резкое мнение о власти, реклама, чужой телефон) дал те же вердикты, что и
// gpt-oss-120b, вдвое дешевле по токенам.
const DefaultModel = "yandexgpt-lite/latest"
