...
// как раз из редкого. ComeToNote с 29.08.2026 тоже ЗАПАСНОЙ путь: настоящая
// величина берётся из `archive.MineComeRate` — знаменатель у неё нашёлся
// (присутствие человека доказывает его же реплика где угодно), и живые
// открывают 14 % заметок своего дня против выставленных здесь 35 %. Совпадение
// не случайно и объясняется плотностью заметок: см. narod.ComeBase.
