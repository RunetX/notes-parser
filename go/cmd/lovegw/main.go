...
func usage() {
  ...
  lovegw platform [-config config.json] [-db lovegw.db] [-archive archive.db] ages               # разовый добор возраста из зеркала и архива (в сеть не ходит)
...
func (d *daemon) setupPlatform(ctx context.Context) error {
	...
	//
	// Клиенты сайта у обхода СВОИ, и это не роскошь: дерево отдаёт только
	// МОБИЛЬНАЯ версия, а с десктопным User-Agent сайт уводит редиректом на
	// полную страницу, где мобильного селектора нет вовсе — обход честно
	// сообщает «вёрстка изменилась?» и не делает ничего (проверено на бою
	// 23.08.2026 общим клиентом зеркала). Пара та же, что у команды reply-scan:
	// второго способа собрать этих клиентов не заводится.
	scanSite, err := replyScanClients(cfg, log)
	if err != nil {
		// Не повод отказывать площадке целиком: без обхода дерево останется
		// угаданным, а зеркало, запись и морда живут дальше.
...
//
// Служба поднимается ВСЕГДА, когда включена площадка, а классификатор
// подключается, только если он настроен и жив: нет ключа — очередь просто
// копится и читается человеком, а это рабочее состояние, а не авария.
//
// До 25.08.2026 у неё была и вторая работа, обязательная независимо от
// классификатора, — доводить проходы по авторам, не уложившиеся в срок
// веб-запроса при отзыве согласия. Отзыв больше не ходит по комментариям
// (обезличивает заметки, а не прячет всё), и доводить стало нечего.
