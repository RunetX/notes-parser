...
func cmdWeb(ctx context.Context, args []string) error {
	...
	srv.SetEvents(pf)
	// Личная переписка (эпик L). Подключается ТОЛЬКО по гейту, и природа у него
	// своя: выключенный означает не «фича на паузе», а «принимать чужие письма
	// площадка не вправе» — написать письмо нечем вовсе, ни маршрута, ни пункта
	// меню, ни кнопки.
	//
	// В БОЮ ГЕЙТ ОТКРЫТ 12.09.2026 решением владельца, и тем же действием правлены
	// два документа оператора: до этого дня «Отказ от ответственности» обещал, что
	// обменяться сообщениями наедине площадка не даёт, а «Политика» не называла
	// писем вовсе. Порядок именно такой — образ с правленными текстами, затем
	// строка в конфиге, один перезапуск на оба, — иначе площадка обещала бы одно и
	// делала другое ровно там, где обещания и есть её единственное основание.
	// Схема, сроки хранения и выдача уполномоченным органам готовы с Ш1:
	// обязанность хранить и обязанность выдать установлены одной статьёй.
	if cfg.Platform.Mail.Enabled {
		srv.SetMail(pf)
	}
