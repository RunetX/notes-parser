...
func cmdWeb(ctx context.Context, args []string) error {
	...
	srv.SetEvents(pf)
	// Личная переписка (эпик L). Подключается ТОЛЬКО по гейту, и выключенная
	// означает не «фича на паузе», а «принимать чужие письма площадка пока не
	// вправе»: пока не закрыт Ш0 (ответ юриста по ч. 4 ст. 10.1 и уведомление
	// РКН — docs/mail-149fz.md), написать письмо нечем вовсе — ни маршрута, ни
	// пункта меню, ни кнопки. Схема при этом накатана, сроки хранения
	// исполняются, а выдача уполномоченным органам готова: обязанность хранить и
	// обязанность выдать установлены одной статьёй.
