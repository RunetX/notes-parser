...
func (d *daemon) setupNarod() error {
	...
	// ТЕМЫ ЗАМЕТКИ разбирает cmd/lovegw, потому что знает оба мира: лексикон и
	// разбор живут в архиве, а ядру народа архив импортировать нельзя. Лексикон
	// ТОТ ЖЕ, каким снят перекос в карточке (см. noteTopics у вакуума): разойдись
	// они, и TopicLift прикладывался бы к теме, которой в замере не было, — а
	// заметить это было бы нечем, кубик молча стал бы звать не тех.
	//
	// До 31.08.2026 темы в бою не подавались вовсе, и рычаг молчал: «кто придёт
	// именно сюда» выходило случайным жребием, ровно как до замера.
	lex := archive.DefaultTopics()
	svc.SetTopics(func(text string) []string {
		t, err := archive.TopicsOf(text, lex)
		if err != nil {
			// Разбор тем — рычаг, а не условие работы: без него кубик про
			// заметку знает лишь то, что она новая, и это ХУЖЕ, но не поломка.
			log.Warn("народ: темы заметки не разобраны", "err", err)
			return nil
		}
		return t
	})
	...
	// НЕМЫЕ ЖИТЕЛИ НАЗЫВАЮТСЯ ВСЛУХ, и это правило про молчание предохранителя.
	//
	// Номер анкеты житель получает ЗДЕСЬ, при сборке службы, из таблицы мира, а
	// заводит его `narod enroll`. Порядок «карточки положили, демона подняли,
	// потом заполнили мир» даёт UserID = 0, и тогда roll молча пропускает
	// жителя («играть ему нечем») — правильно пропускает: публиковать реплику
	// от имени несуществующего автора нельзя. Но снаружи это выглядит как
	// работающая служба, у которой половина народа немая, и в логе про это не
	// было НИ СТРОЧКИ (поймано боем 31.08.2026 при переходе с тридцати жителей
	// на шестьдесят: тридцать карточек загрузились и не сказали ни слова).
	//
	// Отсюда правило шире: предохранитель вправе молчать в работе, но обязан
	// назваться на старте — иначе его тишину не отличить от тишины разговора.
	var mute []string
	for _, p := range players {
		if p.UserID == 0 {
			mute = append(mute, p.Card.ID)
		}
	}
	log.Info("народ подключён", "режим", cfg.Narod.Mode, "жителей", len(players),
		"без_анкеты", len(mute), "мир", cfg.Narod.DBPath)
	if len(mute) > 0 {
		log.Warn("народ: житель без анкеты на площадке молчит — заведите его "+
			"(narod enroll -world <путь к миру>) и перезапустите демона",
			"сколько", len(mute), "кто", strings.Join(mute, ","))
	}
	return nil
}
...
func narodConfig(cfg *config.Config) narod.Config {
	...
	if n.GeneralizeRate > 0 {
		c.GeneralizeRate = n.GeneralizeRate
	}
	if n.LatencyScale > 0 {
